...
def main() -> int:
    ...
    axes = [
        ...
        # 🚨 2026-08-05 신설. 축을 안 만들면 '언제 잰 성적인가'가 화면 어디에도 안 남는다 —
        #   테마는 최근 3년 창으로 채점하는데 그 창이 흐르는 것을 아무도 못 본다.
        {"key": "themes", "label": "테마 명단·성적", "as_of": (load("themes.json") or {}).get("as_of"),
         "cadence": "명단은 수시(손으로) · 성적은 매 갱신 재계산",
         "note": "명단은 사람이 쓴다(build/themes_defs.py) — 자동 추출은 정밀도 27%로 실패했다. "
                 "성적은 최근 756거래일 창에서 매번 다시 잰다. 명단을 결과를 알고 썼으므로 "
                 "정의상 미래참조이며 표시 전용이다",
         "manual": "고정 주기 없음 — 명단이 바뀌거나 가격 패널이 갱신될 때 다시 잰다"},
