...
(적대감사가 잡은 결함). 실측 요약(**2026-07-31 기준**, 채점 모집단까지 좁힌 값):
    선견 지배 … 고베타 94.2%p · 모멘텀 81.4%p(선견 76.9 · 생존 4.5) · 성장 2.4%p
    무영향   … 저변동 0.0 · 가치 +0.1 · 퀄리티 −1.0
⚠ 크기는 점추정으로 읽지 말 것. 구간이 12개월뿐이라 상위 2달이 총편향의 35~65% 를 차지한다
  (퀄리티 65% · 가치 51% · 모멘텀 44%). 방향은 견고하지만 크기는 표본오차와 섞여 있다 —
  |채널| 2%p 미만은 잡음이다.
🚨 그 '잡음'이라는 말이 빈말이 아니다. 두 달(2026-07-29 → 07-31) 만에 이렇게 움직였다:
    성장 26.2 → 2.4%p · 가치 9.5 → 0.1%p · 모멘텀 68.5 → 81.4 · 고베타 86.3 → 94.2.
  12개월 창을 굴리면 앞 달이 빠지고 뒤 달이 들어오는데, 상위 2달이 절반을 차지하니
  그 교체 하나로 값이 배로 바뀐다. **크기를 인용하지 말고 방향과 순위만 읽을 것.**
