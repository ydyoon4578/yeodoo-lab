...
같은 backtest() 코드에 pool_of 만 갈아 끼워 **하나의 패널에서** 네 번 돌린다.
  ...
  published … 주입 전 518 패널·마스크 없음 = 2026-08-23 PIT 전환 **전의** 화면 수치(소급).
              이름만 남았다 — 지금은 하니스 확인용이다(base 와 같아야 한다)
  base      … 주입 후 패널·오늘 518종으로만 제한 = 편향의 기준선(소급)
...
def main() -> int:
    ...
    # ── ① 주입 전 레그 — **주입 전** 깨끗한 518 패널 · 마스크 없음 ─────────────────
    #   2026-08-23 까지는 이것이 배포 수치였고 앵커였다. 지금 배포는 PIT 라 앵커는 pit 레그다
    #   (아래 «앵커 검증»). 이 레그는 base 와 같아야 한다 — 다르면 좁히기가 새고 있다
    #   (channel.harness_zpop). 키 이름 published 는 소비처(style_trails.json 의 pub) 때문에 둔다.
    ...
    #   고침: 편향은 **패널을 고정하고** base(마스크 없음) 대비로 잰다. published 는 하니스
    #   확인용이다(앵커는 2026-08-23 부터 pit 레그).
    SPP.inject(prep)
    ...
    # ── 앵커 검증 — pit 레그가 배포 수치와 같은가(2026-08-23~) ─────────────────
    ...
    # 🚨 2026-09-18 — note 가 «published = 지금 화면에 나가는 수치 · base 가 published 와 정확히
    #   일치한다» 라고 적고 있었다. 2026-08-23 PIT 전환 뒤로 화면(style_perf.json)은 **pit 레그**다
    #   (위 앵커가 강제한다). published 키는 주입 전 레그의 옛 이름으로 남아 그 등식 자체는 여전히
    #   참이지만(하니스 0), 정의와 합쳐 읽으면 «화면 = 소급» 이 됐다 — 바로 옆 수를 거꾸로 설명한 것.
    #   → 뜻을 고쳐 적고, 소급 비교는 캐비엇과 **같은 함수**(ST.pit_shift)로 만든다(두 벌 금지).
    try:
        _shift = ST.pit_shift({"styles": res})
    except Exception as _e:              # 문장 하나 때문에 스타일 사슬을 죽이지 않는다 — 대신 크게 적는다
        _shift = ""
    ...
    doc = {
        ...
        "note": ("같은 백테스트 코드에 pool_of 만 갈아 끼워 유니버스 편향을 잰 것이다. "
                 ...
                 + ((" — 화면에 나가는 스타일 중 차이가 큰 것만 보면 %s이다. " % _shift)
                    if _shift else ". ") +
                 "레그: pit = 선정 시점 멤버 전부(= 화면) · mask = 선정 시점 멤버 ∩ 오늘 · "
                 "base = 오늘 유니버스(소급) · published = 주입 전 패널·마스크 없음(2026-08-23 "
                 "PIT 전환 전에 화면에 나가던 소급 수치 — 키 이름만 남았다). "
                 "채점 모집단까지 그 시점 후보로 좁히므로 base 가 published 와 일치한다"
                 "(잔여 하니스 0 — channel.harness_zpop). "
