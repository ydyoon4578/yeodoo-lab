...
FRED = {
    ...
    # 🚨 2026-09-07 추가 — A7 국면을 **개정 없는 계열로만** 다시 만들려고 받는다.
    #   지금까지 A7 판 넷이 공유한 결함이 «국면 라벨을 오늘의 개정 시계열로 사후에 매긴다»
    #   였다. macro_asof_m 은 **발표 시차만** 밀 뿐 값은 개정본이라, 산업생산·실업률로 매긴
    #   국면에는 개정 선견이 남는다(refresh_regime.py 가 같은 사실을 이미 적어 뒀다).
    #   아래 셋은 전부 **시장가격·정책 공표값**이라 사후 개정이 없다.
    #
    #   DFEDTAR / DFEDTARU — 연준 정책금리 목표. Conover·Jensen·Johnson·Mercer(2008)의
    #     통화조건은 «마지막 정책금리 변경의 **방향**» 이다. 종전 구현은 유효금리(FEDFUNDS)의
    #     12개월 변화 부호를 썼는데 그것은 다른 신호다 — 실측(2006~): 전환 20회 대 6회 ·
    #     긴축 비율 55% 대 38% · 248개월 중 48개월(19%)이 갈린다. 12개월 변화판은 마지막
    #     인상 뒤에도 1년을 「긴축」으로 붙든다.
    #     ⚠ 목표 «범위» 는 2008-12 부터다. 그전은 단일 목표(DFEDTAR)라 **둘을 이어 붙인다** —
    #       유효금리로 «정책 변경» 을 잡으면 문턱(0.25%p 등)이 필요하고 그것이 자유도다.
    #   BAA10Y — 신용 축. ICE 계열(BAMLH0A0HYM2)은 FRED 공개 CSV 가 2023-09~ 3년만 준다
    #     (라이선스). 무디스 Baa − 10년물은 1986년부터 있고 시장가격이라 개정이 없다.
    "DFEDTAR": "연준 목표금리(단일 · ~2008-12)",
    "DFEDTARU": "연준 목표금리 상단(2008-12~)",
    "BAA10Y": "Baa 회사채 − 10년물 스프레드",
}
