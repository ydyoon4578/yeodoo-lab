...
⚠ **style_strategies.pdf 와 종목이 다를 수 있다.** 그 문서는 같은 규칙을 과거로 되돌리는
  ...
  7/10 겹친다. ROE 는 두 출처의 **순위**가 대체로 같다(스피어만 0.91).
  ⚠ 2026-08-05 — 이 자리에 "야후는 평균 자기자본, 랩은 최신 분기"라는 옛 설명이 있었다.
    2026-07-29 에 랩도 평균 자기자본(교과서 정의)으로 바꿨으므로 **분모 정의는 같다.**
    그래도 명단은 갈릴 수 있다 — 45일 지연·유니버스·윈저화가 다르기 때문이다.
    두 문서를 나란히 볼 때는 그 전제로 읽을 것.
