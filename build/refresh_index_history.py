...
⚠ 경계는 **2014-06** 이다. CIK 컬럼이 그때 생겼다.
  🚨 2026-08-11 정정 — 종전 주석은 "2015-03 부터 502/502" 라 적고 START 를 2015-01 로 뒀는데,
    성긴 표본(2011-06 · 2013-06 · 2015-03)만 보고 그 사이를 안 좁힌 것이었다. 다시 재니:
        2014-05 (리비전 2014-04-30)  501종 · CIK **0**
        2014-06 (리비전 2014-05-22)  501종 · CIK **501**
    즉 8개월을 근거 없이 버리고 있었다. START 를 2014-06 으로 내린다.
  ⚠ 여기가 진짜 바닥이다. 명단 자체는 더 내려간다 — 실측으로 SPX 는 2007-04(리비전
    2007-03-26 · 500종), NDX 는 2008-03(리비전 2008-02-28 · 99종·번호목록)부터 파싱된다.
    막는 것은 명단이 아니라 **조인**이다. CIK 가 없으면 티커로만 묶어야 하고, 그러면 개명
    (BBWI←LB · LUMN←CTL …)과 티커 재사용을 구별할 수 없다. 그 대가는 build/pit_backtest.py
    쪽에서 실측했다 — 2009-01 SPX 498종 중 2015 이후 한 번도 안 나오는 109종을 표본
    30종 받아 보니 19종(63%)은 가격이 아예 없고, 남은 11종은 전부 오늘까지 이어진다
    (아직 상장 중이거나 티커 재사용: S→SentinelOne · WB→Weibo · SLE → 다른 것).
    즉 2014-06 아래로는 후보가 다시 생존자로 좁혀져, 생존편향을 재려는 검정이 그 편향을
    도로 갖는다. **자료가 없어서가 아니라 조인이 무너져서** 여기서 멈춘다.
...
START = "2014-06"          # CIK 컬럼이 생긴 첫 달(리비전 2014-05-22). 위 ⚠ 참조.
...
def main() -> int:
    ...
    doc = {
        ...
        "limit": "위키 편집 지연이 있다. 문서 자체가 '현재 기준' 날짜를 본문에 적는 경우도 있어 "
                 ...
                 "CIK 컬럼은 2014-06(리비전 2014-05-22)에 생겨 그 이전은 다루지 않는다 — "
                 "명단 자체는 SPX 2007-04·NDX 2008-03 까지 파싱되지만 티커로만 조인하게 되어 "
                 "개명과 티커 재사용을 구별할 수 없다(실측 근거는 이 파일 머리말).",
