...
STYLE = [("MTUM", "모멘텀"), ("QUAL", "퀄리티"), ("IVE", "가치"), ("USMV", "저변동"),
         ...
         ("SPY", "S&P 500 (기준선)"),
         # 2026-08-13 보강 7종(사용자 요청). 넣기 전에 **초과수익(SPY 차감) 상관**으로
         #   기존 9종과 얼마나 겹치는지 쟀다 — 일간 수익 상관은 주식 ETF 끼리 다 0.9대라
         #   뜻이 없다. 실측(2015-10~2026-08 · 2723일, 가장 닮은 기존 줄):
         #     IVW ↔ RPG   +0.539   가장 안 겹친다
         #     SPMO ↔ MTUM +0.629   ⚠ 랩이 재현한 '방법론'끼리는 0.978 인데 **실제 상품은
         #                            0.629 다.** 상품은 100종을 담고 재현판은 상위 10종이라
         #                            재현판이 서로 훨씬 더 닮는다.
         #     PKW ↔ RSP   +0.769 · SCHD ↔ SDY +0.841 · RPV ↔ RSP +0.847 · VYM ↔ IVE +0.872
         #     SPLV ↔ USMV +0.915  🚨 사실상 같은 줄이다.
         #   눈금: 기존 9종끼리도 IVE↔SDY +0.855 · SDY↔RSP +0.820 이 이미 있다. 그 기준을
         #        확실히 넘는 것은 SPLV 하나뿐이라 나머지는 각자 축이 있다고 보고 넣는다.
         ("SPMO", "모멘텀(S&P)"), ("IVW", "성장(S&P)"), ("RPV", "순수가치"),
         ("SPLV", "저변동(S&P)"), ("SCHD", "고배당+질"), ("VYM", "고배당"),
         ("PKW", "자사주매입")]
