...
# 🚨 2026-09-21 — 126(6개월) → 252(1년). 홈 «기간별 수익률» 차트가 지수 방법론 8종을
#   같이 그리려면 표의 구간을 다 덮어야 한다(1주·1개월·3개월·6개월·1년·올해).
#   126 이면 6개월·1년 칸에서 그림의 끝값이 표와 어긋난다 — 실측 최대 6.98·38.75%p.
#   ⚠ 1일(분봉)과 3년은 여전히 못 덮는다. 그 두 구간에서는 판을 내지 않는다.
PATH_N = 252
...
#   아니게 된 줄 ③ 시점정확으로 재면 짝보다 나쁜 줄. 그 밖은 사용자 결정이다.
# ⚠ **자료·빌더는 그대로다.** 여기 키를 지우면 그 줄이 그대로 돌아온다.
#
# 🚨 2026-09-21 — **사용자 결정으로 진열을 「표준 8종」으로 갈았다.**
#   지시: *"여두 전략 랩 메인이랑 스타일쪽 다 신규로 대체해줘 신규 표준 8종"*
#   나간 셋 `grow`·`hbeta`·`qvm` ← 들어온 셋 `lowvol`·`fcfy`·`netbuy`.
#   고른 기준은 **성적이 아니라 표준 팩터 분류**다(PREREG-2026-09-21-STYLE8ROT §3-1-b):
#     MSCI 6대(가치·규모·모멘텀·퀄리티·저변동·고배당) 전부 + FF5 의 수익성·투자를 대용으로.
#   ⚠ **이것은 2026-08-13 사용자 결정(자사주·현금흐름 계열 셋을 뺀다)을 뒤집는다.**
#     그 셋 중 `fcfy`·`netbuy` 가 돌아오고 `buyback` 은 그대로 남는다
#     (`netbuy` 와 같은 축인데 «얼마를 썼나» 라 학술 축에서 먼 쪽이다).
#   ⚠ **분산이 좋아져서 바꾼 것이 아니다.** 유효 자산 수는 2.02 → **1.95 로 내렸다**
#     (PREREG-2026-09-21-STYLE8ROT-RESULT §4). 바뀐 것은 «표준 축을 덮는다» 하나다.
#   🚨 **이 교체는 `x-style-fmom8` 을 게시한다는 뜻이 아니다.** 그 규칙은 같은 날
#     **기각**됐다(초과 −0.62%p · t −0.41 · 여덟 중 다섯 걸림). 여기서 바뀐 것은
#     **레그 진열**이고, 그 위에 얹는 로테이션은 원장에 기각으로 남아 있다.
HOME_HIDE = {
    ...
    "buyback": "사용자 결정(2026-08-13) · 2026-09-21 에도 유지 — 순발행 축은 netbuy 가 맡는다. "
               "이쪽은 «얼마를 썼나»(매입액÷시총)라 증자로 되돌려도 안 보인다 · 샤프5 0.70",
    # ── 2026-09-21 표준 8종 교체로 새로 빠졌다가, 2026-09-22 에 둘이 돌아왔다 ────
    # 🚨 2026-09-22 사용자 결정 — **「지수 방법론 8종」에 고베타·성장을 더해 10종으로.**
    #   지시: *"지수 방법론 8종에 고베타 s&p500 high beta 랑 성장 s&p 500 growth
    #   (S&P U.S Style) 두개 추가해서 10종으로 만들자"*
    #   그래서 `grow`·`hbeta` 를 이 목록에서 뺀다(= 화면으로 돌아온다).
    #   ⚠ **어제 이 둘을 뺀 근거는 사라지지 않았다.** 그 근거를 지우지 않고 아래에 남긴다 —
    #     성장은 가치(HML)의 공매도 다리이고, 고베타는 저변동(BAB)의 반대 방향이다.
    #     즉 이 10종은 **표준 팩터 축 열 개가 아니라 여덟 축 + 두 축의 반대편**이다.
    #     진열이 넓어진 것이지 분산이 좋아진 것이 아니다.
    #   ⚠ `qvm` 은 **그대로 둔다** — 지시가 둘만 말했다. 셋을 다 되살리지 않는다.
    "qvm": "사용자 결정(2026-09-21) — 퀄리티·가치·모멘텀의 **혼합**이라 표준 축 하나에 대응하지 "
           "않는다. 세 축을 이미 따로 싣고 있다(val·squal·spmo). "
           "2026-09-22 에 grow·hbeta 는 돌아왔지만 이쪽은 지시에 없어 그대로 둔다",
    # ── 2026-09-22 사용자 결정 — 진열에서 빼고 **전략 랩에 맡긴다** ──────────
    #   지시: *"스타일 전략 10종에서 잉여현금흐름이랑 주식수감소는 빼. 이건 그냥 전략 랩에 넣어"*
    #   ⚠ 둘 다 **이미 전략 랩(201종)에 있다** — 새로 만들 것이 없다.
    #     잉여현금흐름 → 「잉여현금흐름 수익률 상위 10」(분기 · PIT)
    #     주식수감소   → 「순주식발행 회피 (희석주식수 증가 최저)」(분기 · PIT)
    #   그래서 여기서 빼는 것은 «진열에서 내린다» 이지 «버린다» 가 아니다.
    #   ⚠ 이것으로 2026-09-21 의 표준 8종 교체가 절반 되돌아간다 — 그때 들어온 셋
    #     (lowvol·fcfy·netbuy) 중 lowvol 만 남는다. 그때의 근거(FF5 의 수익성·투자 축을
    #     대용으로 덮는다)는 지우지 않고 아래 주석에 남긴다.
    "fcfy": "사용자 결정(2026-09-22) — 진열에서 내리고 전략 랩에 맡긴다. "
            "전략 랩의 「잉여현금흐름 수익률 상위 10」이 같은 것을 잰다",
    "netbuy": "사용자 결정(2026-09-22) — 진열에서 내리고 전략 랩에 맡긴다. "
              "전략 랩의 「순주식발행 회피 (희석주식수 증가 최저)」가 같은 것을 잰다",
}
# 🚨 2026-09-22 에 돌아온 둘 — 왜 돌아왔는지 적어 둔다:
#   grow  : 사용자 지시. S&P 500 Growth (S&P U.S. Style) — 공개 산식 지수다.
#           ⚠ 표준 축으로 돌아온 것이 아니다. 가치(val)와 **같은 축의 반대편**이고
#             둘을 나란히 싣는다는 뜻이다. 상관을 보고 읽을 것.
#   hbeta : 사용자 지시. S&P 500 High Beta — 공개 산식 지수다.
#           ⚠ 저변동(lowvol)의 반대 방향이다. 둘이 한 화면에 같이 있다.
#             이것은 «분산» 이 아니라 «양 끝을 같이 본다» 는 뜻이다.
#
# 🚨 위 목록에서 **일부러 뺀 셋** — 2026-09-21 에 화면으로 돌아왔다. 왜 돌아왔는지 적어 둔다:
#   lowvol : 표준 축(저변동/BAB)이라 넣는다. ⚠ 샤프5 0.21 로 18종 **최하위**이고
#            USMV 실물이 홈 ETF 줄에 있다 — 그 사실을 알고도 넣는 것이,
#            이 선정이 성적 기준이 아니라는 가장 분명한 증거다.
#   fcfy   : 수익성(RMW) 축의 **대용**. ⚠ 정확한 대용은 아니다 — RMW 는 매출총이익 기준이고
#            이쪽은 잉여현금흐름이다. 그리고 정본(COWZ)의 분모는 EV 인데 여기는 시가총액이다.
#   netbuy : 투자(CMA) 축의 **대용**(FF 2008 「Dissecting Anomalies」의 순발행).
#            ⚠ CMA 의 정공법은 총자산 증가율이고 그 레그는 아직 없다.
...
def dump_trails(doc):
    ...
    # 🚨 2026-09-21 사용자 지시 «지수 방법론 8개 차트로 맞춰줘» — 홈 «기간별 수익률» 차트의
    #   스타일 판이 **ETF 8종**을 그리고 있어 바로 아래 표의 **지수 방법론 8종**과 다른 것을
    #   보여 주고 있었다. 같은 화면에서 위아래가 다른 것을 말하면 안 된다.
    #   홈이 style_perf.json(97KB)을 받지 않게 **보이는 줄의 경로만** 여기 태워 보낸다.
    # ⚠ 이 파일의 계약은 «선의 끝값 = 아래 표의 그 칸» 이다. 그래서 거래일 오프셋으로
    #   잘랐을 때 trails 와 맞는지 **여기서 검사하고, 어긋나면 안 싣는다.**
    #   실측(2026-09-21): 1일·1주·1개월·3개월·6개월·1년·올해 전부 최대 0.005%p.
    _hide = doc.get("hide") or {}
    _pv = {s["key"]: s.get("path") for s in doc["styles"]
           if s["key"] not in _hide and s.get("path")}
    _pdt = doc.get("path_dates") or []
    bad = []
    for k, pv in _pv.items():
        tr = next((s["trails"] for s in doc["styles"] if s["key"] == k), {}) or {}
        for lab, n in TRAIL:
            v = tr.get(lab)
            if v is None:
                continue
            base = pv[max(len(pv) - 1 - n, 0)]
            if base and abs((pv[-1] / base - 1) * 100 - v) > 0.05:
                bad.append("%s/%s" % (k, lab))
    if bad:
        print("   ⚠ 경로가 표와 어긋난다(%d칸: %s …) — 홈 차트에 안 싣는다"
              % (len(bad), " ".join(bad[:4])))
    elif _pv and _pdt:
        slim["path_dates"] = _pdt
        slim["path"] = _pv
        slim["path_note"] = ("지수 방법론 8종의 **원해상도 일별 경로**(창 시작 = 100). "
                             "홈 차트가 구간별로 **거래일 오프셋**으로 잘라 쓴다 — "
                             "1일 1 · 1주 5 · 1개월 21 · 3개월 63 · 6개월 126 · 1년 252. "
                             "그렇게 자른 끝값이 같은 줄의 trails 와 같음을 빌더가 "
                             "검사하고 실었다(최대 0.05%p 허용).")
        print("   경로 %d종 × %d일 — 홈 차트용(표와 대조 통과)" % (len(_pv), len(_pdt)))
