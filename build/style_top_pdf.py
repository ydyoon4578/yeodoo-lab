...
시점 정확성 — **재무는 시점, 유니버스는 아니다.** 이 구분을 흐리면 안 된다.
  ...
  · 🚨 **유니버스는 오늘의 518종목을 과거로 소급한다 = 생존편향.** 이 랩에는 선정 시점
    멤버십(data/pit_members.json, 사내 DB public.index_constituents, 2020-09~ 월말)이 있고
...
이기 때문에 생기는 필연**이다.
  ...
  ① 데이터 출처. 사이트는 stocks.json 의 **벤더 비율**(fund.roe·pb·tpe·ps)을 그대로 쓴다.
     ...
     P/B 는 두 출처가 거의 같다(배수 1.00~1.06). ROE 는 **레벨이 다르지만 순위는 대체로 같다** —
        ...
        **판정에 쓰이는 것은 순위 쪽**이다.
     🚨 다만 분모 정의가 실제로 다르다. 야후는 **평균 자기자본**을 쓰고 여기서는 최신 분기
        자기자본을 쓴다. 그래서 자사주 매입으로 자본이 줄어든 기업은 이 문서의 ROE 가 높게
        (자본 15%↑ 감소 종목 중위 1.21배), 증자·이익유보로 늘어난 기업은 낮게(10%↑ 증가
        중위 0.85배) 나온다. 교과서 정의는 평균이다 — 바꾸면 순위·성과가 크게 움직이므로
        (실측: 퀄리티 1년 +9.64% → +25.85%) **성과를 보고 고르면 그게 곧 과적합이다.**
        정의 근거로만 결정할 것.
