...
# 🚨 2026-08-19 — 이 전략이 시점정확이 아닌 **진짜 이유**를 여기 적는다.
#   화면(explorer.html)은 이것을 «자산배분이라 종목 축이 없다» 로 찍고 있었는데 거짓이다 —
#   이 전략은 개별 종목을 산다(현재 보유 AAPL·AMD·AMZN…). 사유는 다른 데 있다:
#     후보 풀이 **오늘의 518종**으로 이미 걸러져 있다. build/refresh_13f_history.py 가
#     CUSIP→티커 매핑을 오늘 유니버스로만 만들기 때문에(그 파일 133~135줄), 그때
#     운용사가 들고 있었지만 지금 유니버스에 없는 종목은 이력에서 통째로 빠진다.
#   크기(2026-06-30 실측): 27곳의 13F 신고액 중 유니버스와 겹치는 비율이 중앙 60.3% —
#     **중앙 40%가 후보에서 빠진다.** 페어홈 3.2% · 오크트리 10.4% 처럼 대부분이
#     빠지는 곳도 있다. 그래서 이 전략의 선택은 "그때 신고" 이면서 동시에
#     "오늘 살아남아 지수에 있는 것" 으로 좁혀져 있다.
#   ⚠ 고치려면 유니버스 밖 종목의 CUSIP 매핑과 가격이 있어야 한다 — 별건이다.
PIT_NA = ("후보 풀이 오늘의 유니버스(518종)로 걸러져 있다 — 13F 이력이 CUSIP 매핑을 "
          "오늘 종목으로만 만들어, 그때 들고 있었지만 지금 유니버스에 없는 종목은 "
          "아예 후보가 못 된다. 신고액 기준 중앙 40%가 그렇게 빠진다(2026-06-30 실측). "
          "선택 시점은 시점정확(공시 45일 지연 반영)이지만 후보 풀은 아니다.")
TOPN = 10
...
def main() -> int:
    ...
    r["sid"] = "guru-clone"
    r["pit_na"] = PIT_NA        # 화면이 «왜 시점정확이 없나» 에 이것을 적는다
