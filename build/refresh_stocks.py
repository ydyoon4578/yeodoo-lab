...
def flags(s):
    f = []
    # 🚨 2026-08-05 — 샹들리에 배지(이탈·근접)를 뺐다(사용자 요청으로 칩·프리셋·차트
    #   마름모를 모두 지운 뒤, 이 배지만 남으면 걸러 볼 필터도 눌러 볼 곳도 없는 고아가
    #   된다). chst/chgap/chon 은 sig 에 계속 실린다 — signal_lab 이 chand-lose·chand-back
    #   을 재는 데 쓰고, 그 성적(구별 불가)은 signals.html 이 낸다.
...
def main():
    ...
    if _short:
        ...
        # 10종 이하로 남은 잘림은 중단까지는 아니지만 온전한 이력이 아니다 — 부분 편입으로 강등해
        # 창이 확보된 지표만 내게 한다. (이 줄은 _short 가 있을 때만 뜻이 있다)
        for t in _still:
    ...
    # 2026-07-27 실측: 07-24 가 HUBB 하나(0.2%)뿐이었다. 종목 수·이력 길이·타점 수는 전부
    # 정상이라 기존 완전성 게이트가 하나도 안 걸렸다 — 그것들은 **종목 축**만 보기 때문이다.
    # 결과로 200일 이동평균처럼 연속 창이 필요한 계산이 그 구멍에 걸려 전 종목에서 죽었다.
    #
    # 정상일과 유령일은 아주 잘 갈린다(실측 분포: 정상 0.917~1.000 · 유령 0.002).
    # 구간 초입이 0.917 인 것은 그때 아직 상장 전인 종목이 8% 있어서다 — 정상이다.
    _cov = daily.notna().mean(axis=1)
    _ghost = _cov[_cov < 0.5]
    if len(_ghost):
        print("  유령 거래일 %d일 제거(값 있는 종목이 절반 미만 — 한두 종목의 봉이 만든 날짜):"
              % len(_ghost))
        for _d, _c in _ghost.items():
            _who = list(daily.columns[daily.loc[_d].notna()])[:5]
            print("    · %s 커버 %.1f%% (%s)" % (_d.strftime("%Y-%m-%d"), _c * 100, ", ".join(_who)))
        if len(_ghost) > 20:
    ...
    #   as_of 는 위 1221행에서 **필터 전** 원본의 최대 날짜로 잡힌다. 그런데 바로 위
    #   `daily = daily[_cov >= 0.5]` 가 커버리지 미달인 날을 지운다. 그러면 격자에 없는
    #   날짜가 사이트의 대표 기준일로 나간다.
    #   실측 2026-09-04 러너: 「유령 거래일 1일 제거 · 2026-09-03 커버 **0.6%**(APH·HUBB·VMRK)」
    #   인데 산출물은 「기준일 2026-09-03」이었다 — **518종 중 3종만 가격이 있는 날**을
    #   화면이 기준일이라 말하게 된다.
    #   그리고 그 어긋남이 아래로 번진다: signal_lab 은 pxd_dates[-1](=09-02)을 쓰고
    #   화면 칩은 as_of(=09-03)를 쓰므로 「같은 잡에서 같은 기준일로 굽는다」는 규약이 깨진다.
    #   09-04 에 refresh-stocks 가 그 불일치로 막혔다(2026-09-03 에 넣은 검사가 잡았다).
    #   ⚠ 값을 만들지 않는다 — **격자가 말하는 날**로 맞출 뿐이다.
    if pxd_dates and as_of != pxd_dates[-1]:
        print("  기준일 정정 %s → %s (유령 거래일 제거 뒤 격자의 마지막 날)"
              % (as_of, pxd_dates[-1]))
        as_of = pxd_dates[-1]
    ...
    out = {"as_of": as_of, "source": "yfinance + 표준 테크니컬 (cloud)", "n_stocks": len(stocks), "pxd_dates": pxd_dates,
           ...
           "fundx_note": "이 20개 지표 자체는 랩이 초과수익을 검증한 적이 없습니다(스크리닝 7종은 별도로 "
                         "검증했고 통과 0종). 저PER이 고PER보다 나았는지, 고ROE가 저ROE를 이겼는지 우리는 "
                         "모릅니다. 이 값들은 종목을 이해하기 위한 재무 상태 표시이며, 매수/매도 점수"
                         "(bscore·sscore)·플래그·타이밍·홈 추천·기본 정렬 어디에도 일절 반영되지 않습니다. "
                         "배지는 커버 종목 안에서의 상대 위치(33/67 백분위)일 뿐 ‘싸다=사라’가 아닙니다. 출처 yfinance.",
           "fundx_note_eps": "⚠ 위 EPS 블록은 랩이 백테스트로 검증해 배포한 코어 전략(EPS 리비전 드리프트)이고, 아래 재무 "
                             "표시는 검증 이력이 없는 참고 수치입니다 — 같은 ‘펀더멘털’이라도 지위가 다릅니다.",
