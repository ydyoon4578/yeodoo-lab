...
def main():
    ...
    out = {"as_of": as_of, "source": "yfinance + 표준 테크니컬 (cloud)", "n_stocks": len(stocks), "pxd_dates": pxd_dates,
           ...
           "fundx_note": "랩은 펀더멘털 팩터(밸류에이션·수익성·성장·재무건전성·배당)의 초과수익을 검증한 적이 "
                         "없습니다. 저PER이 고PER보다 나았는지, 고ROE가 저ROE를 이겼는지 우리는 모릅니다. 이 값들은 종목을 "
                         "이해하기 위한 재무 상태 표시이며, 매수/매도 점수(bscore·sscore)·플래그·타이밍·홈 추천·기본 정렬 "
                         "어디에도 일절 반영되지 않습니다. 배지는 커버 종목 안에서의 상대 위치(33/67 백분위)일 뿐 "
                         "‘싸다=사라’가 아닙니다. 출처 yfinance(각 사 최근 분기 보고서 기준이라 결산일이 종목마다 다름).",
