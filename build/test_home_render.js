...
(async () => {
  ...
  //   있는데 안 그렸다' 만 실패로 센다 — 자료 유무를 여기서 먼저 판정한다.
  {
    ...
    // 원/달러는 «기간별 수익률» 지수 차트의 배경선이다(2026-08-23 사용자 요청으로
    //   종전 국채금리 2·10·30년을 대체했다).
    ...
    //     자료(home_perf.series.*.fx)가 있는데 index.html 이 지수 판에 bg 를 안 넘기면 실패.
    {
      const hp = opt("home_perf.json") || {};
      const anyFx = Object.keys(hp.series || {}).some((k) => (hp.series[k] || {}).fx);
      ...
      // 🚨 «blk.fx 가 어디든 나오면 통과» 로 쓰면 안 된다 — 바로 위 주석에 그 글자가 있어
      //   배선을 통째로 지워도 통과한다(실제로 이 검사를 그렇게 썼다가 일부러 부숴 보고
      //   잡았다). **호출에 실제로 넘어가는 식**을 본다.
      if (anyFx && !/\?\{v:blk\.fx\}:null\)/.test(_src))
        fail.push("기간별 수익률 지수 차트에 환율 배경선 배선이 없다 — chart(...,bg) 확인");
      // 그 식이 **지수 판 호출에** 붙어 있는지도 본다(엉뚱한 판에 붙으면 위 검사만으로는 못 잡는다).
      if (anyFx && !/chart\(blk\.dates, ixs, '지수', CIX, null,[\s\S]{0,400}?\?\{v:blk\.fx\}:null\)/.test(_src))
        fail.push("환율 배경선이 지수 판 chart() 호출에 안 붙어 있다");
      // 🚨 옛 금리 배선이 남아 있으면 잡는다 — 사용자가 빼라고 한 것이 조용히 되살아나는
      //   일이 이 저장소에서 실제로 있었다.
      if (/blk\.rt/.test(_src))
        fail.push("지수 차트에 옛 금리 배경선(blk.rt) 이 남아 있다 — 환율로 대체하기로 했다");
      // 첫 화면 기간은 1일이다(사용자 요청 2026-08-23).
      if (!/var STC_HZ='1D'/.test(_src))
        fail.push("기간별 수익률 차트 기본 기간이 1일이 아니다 — STC_HZ 확인");
    }
