...
자료는 커밋한다. 깨진 사실 자체는 Validate site 워크플로가 계속 붉게 띄우므로 숨겨지지 않는다.

🚨 2026-09-03 정정 — 위 마지막 문장이 **반만 참이었다.** 종전에는 「push 마다 도는」이라
  적었는데, GitHub Actions 는 GITHUB_TOKEN 으로 민 푸시에 워크플로를 재귀 실행하지 않는다.
  실측(최근 10일): **봇 커밋 96건 중 validate 런이 있는 것 0건.** 즉 이 관문이 통과시킨
  «이미 있던 결함» 을 보고할 CI 가 봇 커밋 뒤에는 돌지 않았고, 다음에 사람이 밀 때까지
  아무도 안 봤다(중앙 1.5시간 · 최악 5일). 실제로 09-02 refresh-tech 의 기준선 1건이
  3시간 22분 뒤에야 붉게 떴다.
  → 같은 날 validate.yml 에 **예정 실행(매일 09:30 KST)** 을 넣어 그 전제를 참으로 만들었다.
    누가 밀었든 하루 한 번은 절대 기준으로 본다.
  ⚠ 이 관문의 설계 자체는 그대로 옳다. 이 잡과 무관한 결함으로 방금 받은 자료를 버리는
    것이 더 나쁘다 — 다만 그 «딴 데서 잡힌다» 가 실재해야 성립하는 설계다.
