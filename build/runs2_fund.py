# -*- coding: utf-8 -*-
"""build/runs2_fund.py — 런 검정 **반기** 밴드판. 규약은 PREREG-2026-09-20-RUNS2.md.

선행 `runs_fund.py`(분기판)에서 **형성 주기 한 줄만** 바꾼 판이다.
   FORM_M = (3, 6, 9, 12) → (6, 12) · 보유 3개월 → 6개월.
그 밖(252일 창 · z 최하위 10 · 동일가중 · 10/20 밴드 · 왕복 25bp · 잣대 B · 혼합 90/10)은
한 글자도 안 바꿨다 — 등록서 §1 이 «바꾸는 것은 하나뿐» 이라 못박았다.

🚨 F3 문턱이 2.00 이 아니라 **2.24** 다. 같은 축의 두 번째 시도라 본페로니로 올린 값이고,
   결과를 보고 정한 것이 아니다(등록서 §0). F9(회전 4.0회)는 운영 제약이라 안 바꿨다.
🚨 F10 이 새로 있다 — 앞 절반 증분 알파가 음수면 기각.
"""
...
OUT = os.path.join(DATA, "_runs2_fund.json")
...
TURN_CAP = 4.0             # F9 — 운영 제약이라 RUNS 와 같은 값을 쓴다
T_CRIT = 2.24              # F3 — 🚨 두 번째 시도의 본페로니 문턱(등록서 §0)
NSHUF, SEED = 200, 20260920
FORM_M = (6, 12)           # 🚨 반기. 펀드의 형성월 중 둘이라 맞물린다
HOLD = 6                   # 슬리브 보유 6개월
# 🚨 펀드는 **분기 그대로**다. 선행판에서 그대로 옮겼더니 fund_sel 이 FORM_M 을 함께 써서
#   펀드까지 반기가 될 뻔했다 — 그러면 비교 대상이 달라져 이 등록이 통째로 무의미해진다.
#   슬리브만 바꾼다는 등록서 §1 을 코드로 못박는다.
FUND_M, FUND_HOLD = (3, 6, 9, 12), 3
...
def main():
    ...
    for f, g in qg[qg.top30 == "Y"].groupby("ym"):
        if int(f[5:7]) in FUND_M:          # 🚨 펀드는 분기 그대로
    ...
    if "ac" in DG:
        print("   **점수의 반기 자기상관 중앙 %.2f** — 밴드가 걸릴 자리가 있나 (분기판은 0.76 이었다)"
              % DG["ac"].median())
    if "blocked" in DG:
        print("   밴드가 막은 신규 진입 중앙 %.0f종/회차 (최대 %.0f)"
              % (DG["blocked"].median(), DG["blocked"].max()))

    def run(sel, hold=HOLD):
        out, trn, prev = {}, {}, {}
    ...
    f_r, f_trn = run(fund_sel, FUND_HOLD)      # 🚨 펀드는 3개월 보유
    # ── 혼합 ──────────────────────────────────────────────────────────────
    # 🚨 이제 형성월이 어긋난다(슬리브 6·12 / 펀드 3·6·9·12). GURUACC 에서 그런 경우에
    #   «형성월에 명단을 합치면 3개월씩 한쪽 100%» 가 되는 버그를 밟았다.
    #   바른 뜻은 «두 슬리브를 동시에 들고 각자 제 주기로 리밸런스한다» 이고,
    #   겹치는 종목이 없으면 수익률 가중합과 같다. 겹침은 아래에서 실측해 싣는다.
    ix = f_r.index.intersection(r_r.index)
    m_r = (1 - MIXW) * f_r.reindex(ix) + MIXW * r_r.reindex(ix)
    m_trn = None
    ...
    S_r, S_f, S_m = stat(E_r, r_trn), stat(E_f, f_trn), stat(E_m, {})
    # 두 슬리브를 함께 들면 회전도 비중대로 합쳐진다
    S_m["turn"] = (1 - MIXW) * S_f["turn"] + MIXW * S_r["turn"]
    ...
    for nm, s in (("펀드", S_f), ("런검정 반기판", S_r), ("혼합 90/10", S_m)):
    ...
    try:
        ...
        for nm, e in (("펀드", E_f), ("런검정반기", E_r), ("혼합", E_m)):
            cells = []
    ...
    f = {"F1": S_r["y_pp"] <= 0, "F2": (S_m["ir"] - S_f["ir"]) < 0.05,
         "F3": ta < T_CRIT, "F4": pct < 95, "F5": thin / len(DG) > 0.10,
         "F6": OV.w.median() > 50,
         "F7": halves["앞 절반"] * halves["뒤 절반"] <= 0,
         "F8": t8 < 1.5, "F9": S_r["turn"] > TURN_CAP,
         "F10": halves["앞 절반"] < 0}
    txt = {"F1": "단독 초과 ≤ 0", "F2": "IR 개선 < 0.05", "F3": "증분 알파 t < 2.24 (두 번째 시도)",
    ...
    io.open(OUT, "w", encoding="utf-8").write(json.dumps({
        "prereg": "build/PREREG-2026-09-20-RUNS2.md",
        "commit_before": "a65b896d2fcdc0fe1786a87ca13685e71922d742",
        "blob": "3fb35fd89f84e9850691faa1c4707691d2f20a31",
        "const": {"WIN": WIN, "TOPN": TOPN, "BAND": BAND, "COST": COST, "MIXW": MIXW,
                  "TURN_CAP": TURN_CAP, "T_CRIT": T_CRIT, "FORM_M": list(FORM_M),
                  "HOLD": HOLD, "NSHUF": NSHUF, "SEED": SEED},
        "window": {"start": str(j[0]), "end": str(j[-1]), "n": len(j)},
        "stats": {"fund": S_f, "runs2": S_r, "mix": S_m, "noband": S_nb},
        ...
        "series": {"runs2": {str(k): float(v) for k, v in E_r.items()},
