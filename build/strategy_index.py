...
def main() -> int:
    ...
    HIDE_SIDS = {
        ...
        # 🚨 2026-08-23 — **테일 리스크 헤지(a-tail-hedge · r-tail-risk-hedge)를 다시 넣었다**
        #   (사용자 결정). 기각 원장 전수 조사에서 나왔다 — 판정문 스스로 «평가창이 배포북
        #   교집합(2016~)이라 GFC 를 한 번도 관측하지 못했다» 고 적고, 창을 2006-11~ 로
        #   넓히면 **GFC 구간 +17.5%(같은 기간 SPY −50.8%)로 방어가 실측된다** 고 적어 놓았다.
        #   ⚠ 수익 잣대로는 여전히 열위다(샤프 0.659 → 0.422 · 위 주석의 실측). 되살리는 것은
        #     «성과가 좋다» 가 아니라 **이 랩이 위험 축을 따로 재기 때문**이다 — 목적이 낙폭
        #     감축인 규칙을 수익 잣대 하나로 목록에서 지우면, 그 축을 재 놓고 안 보여주는 것이 된다.
