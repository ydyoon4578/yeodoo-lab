...
RULE = ("S&P 500 안에서 **ROE 백분위와 기대투자성장(EG) 백분위를 반씩 평균**해 점수를 "
        ...
        "리밸런스 거래에 **편도 10bp**(살 때 10 · 팔 때 10)를 물린다." % CAP)
...
def diag():
    """진단 3종(build/qg_diag.py) — 구조 · 다중검정 · Funnel."""
    try:
        return json.load(io.open(os.path.join(DATA, "_qg_diag.json"), encoding="utf-8"))
    except Exception:
        return None
...
def _cost_run(R):
    ...
    for k, v in (R.get("runs") or {}).items():
        if "무비용" not in k:
            return v
    return {"quarterly": {"turn": float("nan")}, "monthly": {"turn": float("nan")}}
    ...
    for what, pp, src, verdict, basis, re_ in COSTS:
        ...
        if re_ and R.get("monthly_decomp"):
            ...
            row["note"] = (
                ...
                "잡아 주던 잡음을 도로 집어넣는다."
                # ⚠ 키 이름에 비용률이 들어 있어 상수를 바꾸면 갈린다.
                #   («비용 25bp» 로 박아 뒀다가 10bp 로 내리면서 실제로 깨졌다.)
                #   그래서 «무비용» 이 아닌 쪽을 찾아 쓴다.
                % (d["design"], d["trading"],
                   _cost_run(R)["quarterly"]["turn"],
                   _cost_run(R)["monthly"]["turn"]))
        else:
            row["pp_per_year"] = pp
...
def main():
    ...
    card = {
        ...
        "kind": "basket",
        "rule": RULE,
        # 🚨 2026-09-21 사용자 결정 «해결해» — **발표문을 사실에 맞게 다시 쓴다.**
        #   성적표는 참이고 앵커도 선다(+9.19%p · t 3.37 · qg_cap.series(20) 와 0.00e+00).
        #   틀린 것은 그 수를 **무엇이 만들었나** 에 대한 종전의 서술이었다.
        #   ⚠ 이것은 성적을 깎는 것이 아니다. 같은 수에 다른 이름을 붙이는 일이다.
        "claim": {
            "is": "우량성장 점수로 거른 **대형주 집중 바스켓**. 유효 종목 수는 이름 30개에 "
                  "실질 9.6개이고(1/HHI), 초과의 대부분은 그 집중에서 나온다.",
            "is_not": "종목선별 전략이 **아니다.** 같은 30종을 동일가중으로 들면 초과가 "
                      "+0.35%p · t 0.16 으로 사라진다.",
            "evidence": [
                "같은 30종 · 같은 시점규칙 · 같은 편도 10bp 로 가중만 바꿈 — "
                "지수비중+상한20% +9.19%p(t 3.37) vs 동일가중 +0.35%p(t 0.16). "
                "초과의 **96%** 가 «어느 30종을 고르나» 가 아니라 «누구에게 몰아주나» 에서 온다.",
                "점수 십분위(동일가중·전체 338종) — 1분위 +0.88%p/분기(t 1.90)인데 "
                "10분위도 +0.28%p 이고 1−10 스프레드 t 0.50. **단조롭지 않다.**",
                "크기 구간별(동일가중) — 시총 1~100위 +0.98%p/분기(t 1.35) · "
                "101~250위 +0.12(t 0.13) · 251위~ −0.39(t −0.31). "
                "**점수는 대형주에서만 듣는다.**",
                "IS-OOS 순위상관 ρ 중앙 +0.064 · t 1.23 — 점수가 다음 분기 «순서» 는 못 맞힌다.",
            ],
            # ⚠ 반대쪽 증거도 같이 적는다. 한쪽만 적으면 그것도 발표문이 틀린 것이다.
            "but": "그래도 점수가 죽은 것은 아니다 — **크기를 묶어 놓으면 산다.** 시총 상위 100 "
                   "안에서 점수 상위 30 − 하위 30 이 +9.12%p 이고(각각 +5.77 t 2.16 · "
                   "−3.35 t −1.37), 시총 상위 30 만 사는 대조군은 +3.03%p(t 1.61)에 그친다. "
                   "현행 30종과 시총 상위 30 의 겹침은 중앙 **4종(13%)** 이라 "
                   "«대형주 베끼기» 도 아니다. 점수가 더하는 몫은 +6.17%p 다.",
            "so": "읽는 법 — 이 바스켓을 담는 것은 «좋은 회사를 골라내는 능력» 을 사는 것이 "
                  "아니라 **«우량성장으로 거른 소수 대형주에 집중하는 것»** 을 사는 것이다. "
                  "TE 8.6% · 유효 9.6종목 · 상위 3사 46.1% 가 그 대가다.",
            "verdict": "측정만 — 게시 42종일 때와 같다. 이 랩은 게시 기준을 두지 않고, "
                       "이 재진술도 판정이 아니라 **이름을 사실에 맞춘 것**이다.",
            "unchanged": "성적·창·비용·상한은 하나도 안 바꿨다. 바꾼 것은 문장뿐이다.",
        },
        ...
        "costs": _costs_rows(),
        "limits": [
            # 🚨 2026-09-21 — 이것이 이 카드의 **가장 큰 한계**라 맨 앞에 둔다.
            "🚨 **초과의 96%가 선별이 아니라 가중에서 온다.** 같은 30종을 동일가중으로 들면 "
            "+9.19%p 가 +0.35%p(t 0.16)로 사라진다. 점수 십분위는 단조롭지 않고"
            "(1−10 스프레드 t 0.50), 다음 분기 순위상관은 t 1.23 이다. "
            "⚠ 다만 크기를 묶으면 점수가 산다 — 시총100 중 상위30 − 하위30 +9.12%p. "
            "**즉 점수는 대형주에서만 듣는다**(1~100위 t 1.35 · 101~250위 t 0.13 · 251위~ t −0.31). "
            "이 카드의 `claim` 이 그 재진술이다.",
            ...
            "🚨 **이름은 30개인데 유효 종목 수는 9.6개다**(1/HHI 중앙 · 최근 7.1). "
            "상위 3사 46% 보다 이 수가 정직하다. 참고로 다른 파이프라인의 50팩터 판은 "
            "41.3/50 이다. ⚠ 그리고 **1년 지나면 명단의 4분의 1만 남는다**(Jaccard 0.250).",
            "🚨 **Deflated Sharpe Ratio 가 0.80 이다**(시행 100 가정 · 월 SR 0.307 · T=120). "
            "0.95 문턱을 못 넘는다. 원 문서가 «백 가지 넘는 조합에서 고른 설계» 라고 적었고, "
            "그 자유도를 깎으면 이 정도가 남는다는 뜻이다. ⚠ sd(SR) 근사를 썼다.",
            "🚨 **상한의 최적값이 창마다 갈린다** — 10년 창은 25%, 최근 5년·3년은 20% 다. "
            "지금 20% 인 것은 «최근 창에서 최적이라서» 가 아니라 «20~30% 가 평평해서» 다"
            "(원 등록서 §3). **창을 바꿔 최적을 다시 고르지 않는다.**",
