...
🚨🚨 이 프로브는 **틀렸다.** 지우지 않고 남긴다 — 지우면 다음 사람이 같은 것을 다시 짠다.
   무엇이 틀렸나: 아래 probe_e34 는 `fx/*.json` 을 열어 "rev 와 cogs 가 **둘 다 있는 종목 수**"
   를 셌다. 그건 **파일 단위** 질문이고, 백테스트가 묻는 것은 **월말 단위** 질문이다 —
   "2014-03 시점에, 그때까지 공개된 자료로 채점 가능한 종목이 몇인가".
   태그가 있기만 하면 세었으므로 2019 년부터 시작한 종목도 2012 년 후보로 셈됐다.
   결과(2026-08-12 실측):

       프로브가 말한 것   rev∩cogs 294종 (한 시점 · 2026-06-30)
       실제 이력          2012-06 8종 → 2016-06 13종 → 2020-06 35종 → 2026-06 276종  (34배)
       그래서             x-sugp 는 16.6년이 아니라 **6.1년**만 돌았다(얇은 달 125개 · 후보 중앙 19)

   이 함정은 build/DATA-FACTS.md **#3(후보 풀 램프)** 에 이미 적혀 있었다. 읽고도 이렇게 짰다 —
   문서가 있는 것과 그 문서대로 재는 것은 다른 일이다.

   → 규약: 횡단면 규칙의 프로브는 **반드시 월별(또는 연별) 후보 수를 시계열로** 낼 것.
     한 시점 커버리지는 그 규칙이 **언제부터 돌아가는지**에 대해 아무것도 말하지 않는다.
     아래 probe_insider 의 ④⑤(월별·누적창별 후보 수)가 올바른 본보기다 — 같은 파일 안에서
     한쪽은 제대로 재고 한쪽은 안 쟀다.
   결과 문서: build/PREREG-2026-08-12-INCOME-LINES-RESULT.md §3
