...
무엇을 만드나.
  최근 1개월 **장내** 내부자 거래를 종목별로 합쳐 순매수/순매도 상위를 뽑는다.
...
# 🚨 창은 **1개월**이다(2026-08-12 변경, 이전 182일). 6개월은 지분구조 이벤트 한 건이
#   반년 내내 상위에 눌러앉아 '최근'이 아니게 된다. 실측(기준일 2026-06-30): 182일 4,637건·
#   447종 → 30일 1,547건·226종. 상위 8줄을 채우기에 충분하고, 화면 문구는 이 값에서
#   자동으로 나온다(index.html 이 win_days 를 읽는다) — 숫자와 라벨이 어긋날 수 없다.
WIN_DAYS = 30       # 트레일링 1개월
TOPN = 8            # 홈 카드에 넣을 줄 수
...
def main() -> int:
    ...
    doc = {
        "note": ("최근 1개월 장내 내부자 거래(SEC Form 4 코드 P·S)를 종목별 순금액으로 합친 것. "
