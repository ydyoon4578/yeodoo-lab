...
def build():
    ...
    def s_a7_fidelity():
        ...
        return run_weights(
            ...
            "🚨 2026-09-07 공개 — 이 규칙의 국면 라벨에는 **개정 선견**이 남아 있다. "
            "규칙문의 「발표 30일 지연」은 **날짜만** 민 것이고(macro_asof_m 규약), 값은 "
            "오늘의 개정본이다. 산업생산·실업률은 사후에 개정되고 산업생산은 연례 벤치마크 "
            "개정에서 12개월 변화의 부호가 바뀌기도 한다 — 즉 그때 알 수 있던 라벨이 아니다. "
            "랩이 같은 사실을 regime.json 쪽에는 적어 뒀는데 이 규칙에는 안 달려 있었다. "
            "⚠ 이 사실이 판정을 바꾸지는 않는다(이미 t −0.06 으로 구별 불가다). 다만 화면이 "
            "이 규칙을 «실시간으로 돌린 것» 처럼 말하고 있었으므로 바로잡는다.",
...
def main() -> int:
    ...
    # 🚨 «왜 시점정확이 없나» 를 **원천이** 적는다 — 2026-09-07.
    #   그전까지 이 칸이 비어 있어 화면은 자산 규칙을 그냥 "측정만" 이라 적고 사유를 안
    #   말했다. 빈칸은 «해당 없음» 과 «아직 안 쟀다» 를 구별하지 못한다
    #   (strategy_index._pit_na 머리말이 같은 사고를 기록해 두었다).
    #   ⚠ 화면이 문장을 지어내면 안 된다 — 실제로 13F 복제를 «자산배분이라 종목 축이 없다»
    #     고 잘못 적은 적이 있다. 그래서 여기서 적는다.
    #   ⚠ 여기 한 곳에서만 채운다. 레코드를 만드는 자리가 셋이라(공통 빌더 + 오버나이트 둘)
    #     그쪽마다 적으면 문구가 세 벌이 되고 언젠가 갈린다(되풀이 결함 ③).
    #   ⚠ 손 목록으로 고르지 않는다 — **매매 대상이 자산 패널인 것**만 걸린다. 종목을 사는
    #     규칙(guru-clone)은 holdings.kind 가 달라 안 걸리고, 그쪽은 자기 빌더가 이미 싣는다.
    _PIT_NA_ASSET = (
        "매매 대상이 ETF·지수라 «그때 지수에 있던 종목» 이라는 축이 없다 — 이 랩의 "
        "시점정확 레그는 월별 지수 편입명단으로 종목 후보를 되돌리는 장치인데 여기에는 "
        "되돌릴 명단이 없다. 🚨 그렇다고 편향이 0 이라는 뜻은 아니다. 오늘 남아 있는 ETF "
        "만 패널에 있으므로 상장폐지된 ETF 는 애초에 후보에 없다 — 그 축은 이 랩이 아직 "
        "안 잰다. 「해당 없음」이 아니라 「다른 축이고 미측정」이다.")
    _na = 0
    for _r in rows:
        if _r.get("pit_na"):
            continue                       # 자기 사유를 이미 실은 규칙은 안 덮는다
        if ((_r.get("holdings") or {}).get("kind")) == "asset":
            _r["pit_na"] = _PIT_NA_ASSET
            _na += 1
    print("  ~ 시점정확 미측정 사유를 %d종에 실었다(자산 패널 매매)" % _na)
