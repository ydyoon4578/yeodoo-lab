...
한계는 결과와 함께 싣는다(생존편향·표본 길이·비용 0). 좋은 것만 고르지 않고 전부 게시한다.
...
def run():
    ...
    # 🚨 2026-08-17 사용자 지시 — **|t| 임계 2.0 을 걷었다.**
    #   ⚠ 2026-08-13 에는 «화면이 신호마다 쓴다/안 쓴다를 골라야 하니 문턱이 없으면 화면이
    #     성립하지 않는다» 며 관례값 2.0 으로 고정해 두었다. 그 전제가 틀렸다 — 화면은
    #     **잰 값을 보여 주면** 성립한다. 랩 본편이 t 에 선을 안 긋기로 한 마당에
    #     이 페이지만 긋고 있으면 같은 저장소가 두 가지 잣대를 쓰는 것이다.
    #   무엇으로 가르나. **부호와 강건성만** 쓴다 — 둘 다 기준이 0 이라 «고른 선» 이 아니다
    #     (x-clv 의 «문턱 0 은 정의상의 값이다» 와 같은 성격).
    #       · 관례 방향인가  : t 의 부호가 그 신호의 교과서 방향과 같은가
    #       · 꼬리에 기대나  : 상위 5% 사건을 빼도 귀무보다 나은 쪽에 남는가
    #     크기는 **표의 t·p 칸이 그대로 말한다.** 어느 크기부터 «유효» 인지는 안 정한다.
    #   ⚠ 표본 부족(MIN_EVENTS)은 남긴다. 그건 판정선이 아니라 **자료 충분성** 규칙이고,
    #     이 랩은 커버리지 게이트를 판정 게이트와 구별해 왔다(XSEC_MIN_POOL·MIN_COV).
    for r in out:
        m = r.get("h%d" % PRIMARY)
        if not m:
            r["verdict"] = "표본 부족"
        ...
        _t = "" if m["t"] is None else " (t %+.2f)" % m["t"]
        if agree and robust:
            r["verdict"] = "관례 방향"
            r["use"] = ("t 의 부호가 교과서 방향과 같고, 상위 5%% 사건을 빼도 귀무보다 나은 쪽에 "
                        "남는다%s. 크기가 쓸 만한지는 t·p 칸을 직접 볼 것 — 이 랩은 «여기부터 "
                        "유효» 라는 선을 긋지 않는다." % _t)
        elif agree and not robust:
            r["verdict"] = "관례 방향 · 꼬리 의존"
            r["use"] = ("부호는 교과서 방향인데, 상위 5%% 사건을 빼면 귀무보다 나은 쪽에 남지 "
                        "못한다%s — 그 몇 건을 못 잡으면 없는 엣지다." % _t)
        else:
            # 관례와 반대 — 이 랩에서 가장 쓸모 있는 칸이자 가장 위험한 칸이다.
            r["verdict"] = "관례 반대 방향"
            r["use"] = ("t 의 부호가 교과서 방향과 «반대»다%s. 뒤집으면 쓸 수 있다는 뜻이지만, "
                        "방향을 결과를 보고 정하는 것이므로 발견이 아니라 가설이다. " % _t
    ...
    def port(rank_df, asc, label, rule, why):
        ...
        🚨 2026-09-19 — `i - 1` 이었다(AUDIT-2026-09-19-SIGLOOK). 그러면 순위를 낸
           **그 월말 종가**에 갈아타고 곧바로 R[i](= 종가 m → 종가 m+1)를 먹는다.
           하루가 통째로 앞선다. 같은 파일 328행의 사건 연구는 `P.shift(-H-1)/P.shift(-1)`
           로 **다음날 진입**을 지키고 있었고, protocol[0] 도 그렇게 적혀 있다 —
           **한 파일이 두 잣대를 쓰고 있었다.**

           이 하루가 작지 않다. 순합의 순위는 역추세 신호가 절반이고, 신호가 세게
           발동한 날의 다음날이 되돌림이 가장 큰 날이다. PREREG-2026-09-19-SWING1020
           에서 같은 결함을 고치니 연 +7.09% → +3.51% 로 **수익의 절반**이 사라졌다.

           `i - 2` 로 미루면 월말 m 의 순위로 m+1 종가에 진입하고 처음 먹는 수익이
    ...
    ports = [
        port(NET, False, "신호 합의 상위 %d (관례 방향)" % TOPN,
             "월말에 순합의(매수신호 − 매도신호)가 가장 높은 %d종목을 판정 다음 거래일 종가로 사서 동일가중 보유." % TOPN,
             "교과서대로라면 매수 신호가 몰린 종목이 좋아야 한다. 그 명제를 포트폴리오로 직접 건다."),
        port(NET, True, "신호 합의 하위 %d (역방향)" % TOPN,
             "월말에 순합의가 가장 낮은(매도 신호가 몰린) %d종목을 판정 다음 거래일 종가로 사서 동일가중 보유." % TOPN,
             "위와 정반대. 개별 신호 표에서 매수 신호들이 일관되게 음(−)이었으므로, "
             "그 방향이 포트폴리오에서도 살아남는지 본다. ⚠ 방향을 결과를 보고 정했다 — 가설이다."),
        port(NB, False, "매수 신호 최다 %d" % TOPN,
             "월말에 매수 신호가 가장 많이 발동한 %d종목을 판정 다음 거래일 종가로 사서 동일가중 보유(매도 신호는 무시)." % TOPN,
    ...
    doc = {
        ...
        "horizons": list(HOR), "primary": PRIMARY,
        "bench": "동일가중 유니버스(β조정)",
        "protocol": [
            ...
            "상위 5% 사건을 뺀 평균을 함께 낸다. 그 값이 귀무보다 나은 쪽에 안 남으면 "
            "'관례 방향 · 꼬리 의존'으로 적는다.",
            "🚨 |t| 임계를 걷었다(사용자 지시 2026-08-17). 종전에는 관례값 2.0 을 고정 문턱으로 "
            "썼고 그 위를 '관례대로 유효'라 불렀다. 지금은 «어느 크기부터 유효인지 정하지 "
            "않는다» — 부호(교과서 방향인가)와 강건성(꼬리를 빼도 남는가)만 적고, 크기는 "
            "t·p 칸을 그대로 싣는다. 랩 본편이 t 에 선을 안 긋는데 이 표만 그으면 한 저장소가 "
            "두 잣대를 쓰는 것이 된다.",
            "⚠ 선이 없어졌다고 우연이 없어진 것은 아니다. 신호 %d개를 같은 표본에서 쟀으므로 "
            "영가설에서도 |t| 가 2 를 넘는 것이 약 %d개 나온다 — 크기를 볼 때 이 사실을 "
            "같이 볼 것." % (len(judged), round(len(judged) * 0.05)),
            "표본이 %d건 미만이면 판정하지 않고 '표본 부족'으로 남긴다." % MIN_EVENTS,
        ],
        "limits": [
            "생존편향 — 오늘의 %d종목을 과거에 그대로 적용한다. 그 사이 상장폐지·편출된 종목이 "
            "없어 모든 수치가 실제보다 좋게 나온다." % len(tick),
            # ⚠ 기간도 국면 성격도 숫자로 박지 말 것 — 패널을 늘리면 그대로 거짓말이 된다.
            #   '대부분이 강세장'은 3년 구간의 사실이었을 뿐, 2018·2020·2022를 포함하면 거짓이다.
            "표본은 %s(%s~%s)이다. 이 구간이 겪은 국면이 결과를 좌우하므로, 특히 매도 신호는 "
            "구간 성격에 민감하다." % (_span_txt, dates[MIN_HIST], dates[-1]),
    ...
    _ts = [r["h%d" % PRIMARY]["t"] for r in out
           if r.get("h%d" % PRIMARY) and r["h%d" % PRIMARY].get("t") is not None]
    print("신호 %d개 · 분류 %s · |t| 최대 %.2f (임계 없음 — 2026-08-17)"
          % (len(out), vc, max((abs(x) for x in _ts), default=0.0)))
