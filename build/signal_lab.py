...
def run():
    ...
    # 🚨 2026-08-13 사용자 지시 — n 에 따라 조여지는 문턱을 쓰지 않는다.
    #   종전: z_of(0.05/len(judged)) — 신호를 하나 더 잴 때마다 전원의 문턱이 올라갔다.
    #   ⚠ 여기서는 문턱을 **관례값 2.0 으로 고정**한다(없애지 않는다). 이 랩은 신호마다
    #     '쓴다/안 쓴다'를 골라야 하는 화면이라, 문턱이 아예 없으면 그 화면이 성립하지 않는다.
    #     대신 고정했다는 사실과 그 대가(신호 %d개면 영가설에서도 약 5%%가 넘는다)를 적는다.
    tcrit = 2.0
    for r in out:
        m = r.get("h%d" % PRIMARY)
        if not m:
    ...
    doc = {
        ...
        "protocol": [
            ...
            "통과시키지 않는다.",
            "🚨 다중검정 보정을 껐다(사용자 결정 2026-08-13). 임계는 관례값 |t|≥%.1f 로 "
            "**고정**이고 신호 수에 따라 조이지 않는다. 신호 %d개를 같은 표본에서 쟀으므로 "
            "영가설에서도 약 %d개가 이 임계를 넘는다 — 넘었다는 것이 곧 발견은 아니다."
            % (tcrit, len(judged), round(len(judged) * 0.05)),
