...
OVERRIDE: dict = {
    ...
    # 🚨 2026-09-06 — 대기열 11종을 **한 장씩 열어 읽은** 결과. 낱말 판정으로는 못 가른다.
    #   ⚠ 이 작업 자체가 결론이다: 「구현 대기」라 적힌 것의 절반 이상이 실은 못 하는 것이었다.
    #     하나씩 집어 구현하다 알게 되면 매번 절반이 헛걸음이라, 먼저 훑는 편이 싸다.
    "A14": {"verdict": "underspec", "hits": ["예: 각 4종", "등"],
            "why": "크기가 「**예:** 각 4종, 합계 12종」이라 예시이고, 팩터 목록도 "
                   "「B/P·FCF수익률·GP/자산·ROE·리비전 강도 **등**」으로 열려 있다. "
                   "무엇을 몇 개 쓸지 내가 정하면 자유도를 만드는 것이다"},
    "A18": {"verdict": "underspec", "hits": ["그래뉼러 충격이 낮은"],
            "why": "집중도 게이트(상위 1/3)와 바스켓(시총 하위 40%)은 구체적인데, 두 번째 "
                   "조건 「그래뉼러 충격이 **낮은** 달」의 문턱이 없다. 그것을 정하는 순간 "
                   "게이트가 내 손에서 나온다"},
    "C15": {"verdict": "underspec", "hits": ["퀄리티·밸류 포트폴리오"],
            "why": "정크 바스켓(수익성 하위 30% ∩ 공매도잔량 상위 30%)은 구체적이다. "
                   "그런데 카드의 산출물은 **게이지(상태변수)** 이고, 그 게이트를 «퀄리티·밸류 "
                   "포트폴리오»에 건다는데 랩의 어느 규칙인지 지정이 없다"},
    "D14": {"verdict": "not_a_rule", "hits": ["관측만 한다"],
            "why": "전략이 아니라 **감사**다 — 「각 팩터 포트폴리오를 규칙 변경 없이 그대로 둔 채 "
                   "관측만 한다」. 산출물이 기여도 분해이지 보유 명단이 아니라 이 랩의 "
                   "백테스트 틀에 담을 것이 없다. ⚠ 값이 없다는 뜻이 아니다 — 별도 분석의 일이다"},
    "B11": {"verdict": "no_input", "hits": ["특허 클래스 분포"],
            "why": "특허 **클래스 분포 벡터**가 필요한데 산출물이 없다(data/ 에 특허 파일 0건). "
                   "probe-patents.yml 은 «자료 관문 실측» 1회성이라 벡터를 안 만든다"},
    "E20": {"verdict": "no_input", "hits": ["자사주매입 승인 공시"],
            "why": "이벤트가 **자사주매입 승인 공시(8-K)와 그 규모**인데, data/filings.json 은 "
                   "제출 사실만 담고 본문을 파싱하지 않는다(규모 ≥ 발행주식수 5% 필터를 못 건다)"},
    "E28": {"verdict": "no_input", "hits": ["TOPIX 프라임"],
            "why": "**일본 유니버스**(TOPIX 프라임)와 JPX 공시 명단이 필요하다. "
                   "이 랩의 유니버스는 S&P 500 ∪ NASDAQ 100 뿐이다"},
    "E36": {"verdict": "no_input", "hits": ["판매관리비", "매출채권", "재고"],
            "why": "여섯 신호 중 넷이 **판매관리비(SG&A)·매출채권·재고**를 쓰는데 SEC 재무 "
                   "태그 22종에 셋 다 없다(sga·recv·inv 실측 부재). "
                   "⚠ 랩에 x-cdisc(매출증가 − 원가증가)가 있는데 그것은 cogs 로 만든 다른 축이다"},
