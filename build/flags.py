...
def main() -> int:
    ...
    def f_rsi2(i):
        ...
        if r is None or m2 is None:
            return None, None
        above = PX[i] > m2
        # 🚨 MA200 조건은 ON 과 주의 **둘 다** 에 건다. 처음에 주의에서 빠뜨렸다 —
        #   원문 설명이 «주의 <20 · ON <10 (&종가>MA200)» 이라 괄호가 ON 에만
        #   붙은 것처럼 읽혔는데, 원천 구현은 두 단계 모두에 걸고 있었다.
        #   이 조건이 이 신호를 «상승추세 속 눌림목» 으로 만든다. 빼면 하락장에서
        #   떨어지는 칼을 잡는다(원천 주석도 «2022형 하락추세선 반복 손실»).
        s = "ON" if (r < 10 and above) else ("주의" if (r < 20 and above) else "OFF")
    ...
    FLAGS = [
        ("vix_spike", "VIX 스파이크", f_vix, "주의 25 또는 1d+15% · ON 35 또는 +30%",
         "패닉 급등 → 역발상 매수 검토"),
        # ⚠ 문구는 원천보다 정확하게 적는다 — 원천은 «(&종가>MA200)» 를 ON 뒤에만
        #   붙여 두 단계 모두에 걸린다는 사실이 안 보인다. 랩 문구는 코드대로 적는다.
        ("rsi2", "RSI-2 과매도", f_rsi2, "주의 <20 · ON <10 (둘 다 종가>MA200)",
         "단기 과매도 반등"),
