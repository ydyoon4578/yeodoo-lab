...
🚨 처음에 '못 한다'고 판단했는데 그건 **추정량이 나빴던 것**이지 자료가 없어서가 아니었다.
  ...
  · **주가 기준** = 그 국면의 섹터 월평균 − **같은 국면의 유니버스 동일가중 월평균**.
    (2026-08-13 정정 — 여기 "SPY 월평균"이라 적혀 있었으나 코드는 :315 에서 allr,
     즉 같은 달 전체 518종 동일가중을 뺀다. 화면 문구도 같은 오기를 갖고 있었다.)
...
# 🚨 2026-08-11 **두 번째 교정** — 0.22 → 0.02(증시 곡선 바닥). 앞 교정을 되돌린다.
#   앞 교정의 논리는 "7국면은 성장×물가로 정의된 경제 상태이니 경기 곡선에 맞춰야 한다"
#   였고 정의로는 맞았다. 그런데 **실측이 반대를 말한다.**
#   사용자 지적: "15개월째 회복인데 SPX 는 30% 가까이 올랐다. 그런데 표시가 증시 정점보다
#   뒤에 있다 — 이걸 보고 15개월 전에 비중을 줄였으면 크게 잃었을 것이다."
#   그래서 국면별 **이후 SPY 수익**을 쟀다(2009-01~2026-08 · 212개월 · 중앙값):
#       침체 +21.6% · 연착륙 +21.0% · 후기사이클 +16.3% · 골디락스 +15.5% ·
#       회복 +15.3% · 과열 +2.7%   (전체 중앙 +16.0%)
#   회복 이후 12개월이 전체 중앙과 사실상 같은데 그림은 증시 정점 **뒤**에 놓아
#   '고점 지났다' 로 읽혔다. 그림이 실측과 반대를 말하고 있었다.
#   0.22 배치에서 선도수익 최대는 0.22, 최소는 0.72 — 증시 바닥(0.02)·정점(0.52)에서
#   각각 정확히 0.20 어긋난다. 0.02 로 되돌리면 **선도수익이 증시 바닥에서 최대,
#   증시 정점에서 최소**가 되어 여섯 칸이 코사인 모양으로 맞는다.
#   ⚠ 그래서 이 그림의 7국면은 **증시 곡선 기준**이다. 경기 곡선 기준이 아니다 —
#     읽는 사람이 행동하는 축이 증시이고, 실측 선도수익도 그쪽에 맞기 때문이다.
#     대가: 골디락스(확장·저물가)가 경기 곡선에서는 아직 하강 구간에 놓인다. 알고 고른 것이다.
#   ⚠ 선도수익은 **겹치는 표본**이다(월별 212개 관측이지만 12개월 창이 겹친다).
#     그리고 이 랩의 정식 검정은 7국면이 다음 달을 못 가른다는 것이다(p 0.152).
#     배치 근거로만 쓰고 예측력의 증거로 쓰지 않는다.
ECO_SHIFT = 0.02            # 증시 곡선 바닥. 여기를 바꾸면 7국면이 통째로 따라 움직인다.
_PHASE0 = [
...
#   내용을 옮겨 적은 것이다. 화면이 그 사실을 말하고, 아래 cmp 가 이 랩의
#   실측과 얼마나 겹치는지를 같이 낸다 — 겹치지 않으면 겹치지 않는 대로 보인다.
# 🚨 두 칸은 **원 도표에 없던 것을 더한 것이다**(사용자 결정 2026-08-12) —
#   강세장에 커뮤니케이션 · 약세장 후반에 부동산.
#   원 도표가 그것들을 뺀 것이 아니라 **그때는 없던 섹터다.** GICS 가 부동산을
#   금융에서 떼어낸 것이 2016-08, 커뮤니케이션을 통신+미디어로 다시 세운 것이
#   2018-09 인데 이 도표는 그보다 오래됐다. 11섹터 중 둘만 어느 칸에도 안 적혀
#   있으면 '이 도표가 두 섹터를 뺐다'로 읽히는데, 그것이 아니다.
#   ⚠ 그래서 이 목록은 이제 '그대로 옮긴 것'이 아니다. 화면 설명문도 같이 고쳤다
#     (regime.html · '그대로 옮긴 것' → '옮기고 두 섹터를 더한 것').
#   ⚠ cmp 의 분모는 그대로 3 이다. 그쪽이 세는 것은 **실측 상위 3**(TOPN)이지
#     교과서 목록의 길이가 아니다 — 목록이 넷이 돼도 겹칠 수 있는 최대는 셋이다.
...
REF_BOXES = [
    ...
    (0.27, "강세장",     ["기술", "산업재", "소재", "커뮤니케이션"],
     "Technology · Industrials · Basic Materials · Communication Services"),
    (0.52, "증시 정점",  ["소재", "에너지", "필수소비"],       "Basic Materials · Energy · Staples"),
    (0.77, "약세장",     ["에너지", "필수소비", "헬스케어"],    "Energy · Staples · Healthcare"),
    (0.96, "약세장 후반", ["헬스케어", "유틸리티", "금융", "부동산"],
     "Healthcare · Utilities · Finance · Real Estate"),
]
...
STYLES_ETF = [("SPMO", "모멘텀"), ("IVW", "성장"), ("IVE", "가치"), ("QUAL", "퀄리티"),
              ("SPLV", "저변동"), ("SCHD", "고배당"), ("SDY", "배당성장"), ("RSP", "동일가중")]
# 🚨 2026-08-13 — 17종에서 **8종으로** 줄였다. 홈 표·홈 시계열과 같은 목록이다.
#   사용자가 "이건 안 쓸 거야" 로 여덟(VYM·MTUM·USMV·PKW·RPG·RPV·SPHB·SIZE)을 걷었고,
#   한 사이트에서 같은 이름의 묶음이 화면마다 다른 목록이면 그게 곧 다음 사고다.
# ⚠ 잃은 것을 적어 둔다: 뺀 것 중 RPV·VYM·PKW 는 2006 부터라 침체 표본이 n=18 이었다.
#   남은 8종 중 2006 부터인 것은 IVE·IVW·RSP·SDY 넷이고, SPMO(2015-11)·QUAL(2013-08)·
#   SPLV·SCHD(2011)는 2009 침체를 못 겪었다. 침체 열이 그만큼 얇아진다 — 각 칸의 n 이
#   그 사실을 그대로 말한다(주의 문구도 자료에서 다시 만들어진다).
STYLE_BENCH = "SPY"
STYLE_MIN_N = 3
...
def main():
    ...
    # ── 국면별 **이후** SPY 수익 — 자리만으로 읽게 두지 않는다 ────────────
    # 🚨 그림의 자리는 통념이 정한 것이고, 사람은 그 자리를 보고 행동한다. 그래서 각 칸에
    #   '그 국면에 있을 때 실제로 뭐가 일어났나' 를 같이 싣는다. 자리가 말하는 것과 실측이
    #   다르면 독자가 그 자리에서 바로 볼 수 있어야 한다(2026-08-11 사용자 지적).
    # ⚠ 이름을 FWDRET 로 둔다 — drift 의 카운터 `fwd` 와 겹쳐 조용히 덮였던 자리다
    #   (실측: 아래 `fwd = back = 0` 이 이 dict 를 정수로 바꿔 버렸다).
    # ⚠ 겹치는 표본이다(12개월 창이 서로 겹친다). 정식 검정은 growth_split 쪽이고
    #   7국면은 다음 달을 못 가른다(p 0.152). 서술이지 예측이 아니다 — 화면이 그렇게 적는다.
    FWDRET = {}
    try:
        _A = json.load(io.open(os.path.join(DATA, "assets.json"), encoding="utf-8"))
        _ad, _sp = _A["dates"], (_A.get("px") or {}).get("SPY") or []
        _me = {}
        for _i, _x in enumerate(_ad):
        ...
        _acc = {}
        for _h in (d.get("history") or []):          # 창을 안 자른다 — 표본이 클수록 낫다
            _i = _me.get(_h["dt"][:7])
            if _i is None:
                continue
    ...
    out = {
        ...
        "fwd_note": ("국면별 **이후** SPY 수익(중앙값 · 전체 이력). 겹치는 표본이라 검정이 "
                     "아니라 서술이다 — 이 랩의 정식 검정은 7국면이 다음 달을 못 가른다는 "
                     "것이다(p 0.152). 배치가 말하는 것과 이 숫자가 다르면 숫자를 믿을 것."),
        "fwd": FWDRET,
        ...
        "sectors": {"price": by_price, "earn": by_earn, "topn": TOPN, "earn_meta": emeta,
                    # 🚨 2026-08-13 — 여기 "SPY 월평균"이라 **손으로 적혀 있었다.** 실제로
                    #   빼는 것은 SPY 가 아니라 같은 달 전체 518종 동일가중이다(위 :315
                    #   `series[m] - allr[m]`). 같은 오기가 세 벌이었다 — 이 줄 · 모듈
                    #   독스트링 · regime.html:1342. 화면은 그중 하나를 읽고 다른 하나를
                    #   바로 아래 문단에 적어, 인접한 두 문장이 서로를 부정하고 있었다.
                    #   ⚠ 이제 대조군 이름을 **한 곳(pmeta["bench"])에서만** 만든다.
                    "price_basis": ("그 국면의 섹터 월평균 − 그 국면의 %s 월평균(%%p)"
                                    % pmeta.get("bench", "대조군")),
