...
def build(J, X):
    ...
    h.append("<div class='box'><b>왜 이 전략 하나인가.</b> 펀드 틀(S&amp;P500 90%% + 바스켓 10%%)로 잰 전략 여섯 가운데 EG30 과 우량성장선별 30 이 가장 높았고, "
             "둘은 월 초과 상관 0.70 · 초과 차이 연 0.00%%p 로 사실상 같은 전략이라 더 단순한 EG30 을 남겼다(금융 판정은 시점정확으로 고쳤다). "
             "그 위에 모멘텀 통합 둘을 결과를 보기 전에 등록해 짝지어 비교했고(부록 A), 다른 전략과 반반 섞기 넷을 같이 쟀다(부록 B). <b>%s</b></div>" % esc(verdict_line(J)))
    if is_mix:
    ...
    lims = ["<b>백테스트다.</b> 120개월 · IR 표준오차 약 0.32 — 전체 IR %s 의 95%% 구간은 대략 %s ~ %s." % (num(A["ir"]), num(A["ir"] - 0.63), num(A["ir"] + 0.63)),
            ...
            "<b>금융 판정.</b> 랩에는 그때의 GICS 자료가 없어 금융 경계를 건드린 두 변경(2016-09 부동산 분리 · 2023-03 결제 처리 이동)만 손으로 되돌렸다.",
