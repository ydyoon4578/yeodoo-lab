...
GURUS = {
    ...
    1709323: "히말라야 캐피털 (리루)",
    1112520: "에이커 캐피털 (에이커)",
    1358706: "에이브럼스 캐피털 (에이브럼스)",
    1056831: "페어홈 (버코위츠)",
    1720792: "루안 커니프 (세쿼이아 펀드)",
    1096343: "마클 그룹",
    # 2026-09-23 추가(사용자 «넣을만한거 다 넣어보자») — CIK 는 SEC 제출 문서 경로·13F 추적 사이트로 하나씩 확인
    1549575: "달랄 스트리트 (파브라이)",
    860643: "가드너 루소 & 퀸 (루소)",
    1036325: "데이비스 셀렉티드 (데이비스)",
    813917: "해리스 어소시에이츠 (오크마크)",
    807985: "사우스이스턴 (호킨스·롱리프)",
    1697868: "밸리 포지 캐피털 (칸테사리아)",
    915191: "페어팩스 파이낸셜 (왓사)",
    1569205: "펀드스미스 (테리 스미스)",
    1649339: "사이언 애셋 (버리)",          # 2025-11 투자자문 등록 해지 — 마지막 13F 는 2025-09-30
    # ── 행동주의 — 지분을 들고 경영에 개입한다 ──────────────────────────
    2026053: "퍼싱스퀘어 (애크먼)",
    1040273: "서드포인트 (롭)",
    1791786: "엘리엇 (싱어)",
    1345471: "트라이언 (펠츠)",
    1418814: "밸류액트",
    921669: "아이칸 (칼 아이칸)",          # 개인 명의 제출(ICAHN CARL C) — 옛 법인 1412093 은 DROPPED
    1517137: "스타보드 밸류 (제프 스미스)",
    1998597: "자나 파트너스 (로젠스타인)",      # 2023-09~ 새 법인 — 옛 1159159 는 PREDECESSOR 로 잇는다
    1489933: "그린라이트 캐피털 (아인혼)",      # 2024~ DME Capital Management 가 합산 제출 — 옛 1079114 는 PREDECESSOR
    # ── 성장(타이거 계열) ────────────────────────────────────────────
    1167483: "타이거 글로벌 (콜먼)",
    1061165: "론파인 캐피털 (맨델)",
    1103804: "바이킹 글로벌 (핼버슨)",
    ...
    1697748: "아크 인베스트 (우드)",
    1747057: "D1 캐피털 (선드하임)",
    1541617: "알티미터 캐피털 (거스트너)",
    1798849: "듀러블 캐피털 (엘런보겐)",
    934639: "매버릭 캐피털 (에인슬리)",
    1602189: "드래고니어",
    1088875: "베일리 기포드",
    # ── 매크로 ──────────────────────────────────────────────────────
...
# ── 축(2026-08-19 사용자 결정 «축에 따라 다르게 관리») ────────────────────
# 🚨 축은 «성적» 이 아니라 **성격** 이다. 이 랩은 이들의 성과를 검증한 적이 없다.
# 🚨 overlap=False 인 축은 **공통보유·겹침 전략 계산에서 뺀다.** 왜 그래야 하나 —
#   퀀트는 13F 가 수천~수만 종목이다(실측 2026-06-30: AQR 21,077 · 르네상스 3,140 ·
#   고담 1,791). 지금 명단의 나머지는 9~90종이다. 사실상 유니버스를 통째로 드는
...
AXES = [
    ("value",    "집중 밸류",    "소수 종목을 오래 든다",              True,
     [1067983, 1061768, 1709323, 1112520, 1358706, 1056831, 1720792, 1096343,
      1549575, 860643, 1036325, 813917, 807985, 1697868, 915191, 1569205, 1649339]),
...
#   **신규 13종**으로 찍힌다 — 애크먼이 새로 산 것이 아니라 법인이 바뀐 것인데
#   화면은 «이번 분기에 13종을 새로 샀다» 고 말하게 된다. 그건 거짓이다.
#   → 변동을 잴 때만 옛 법인의 같은 분기를 본다. 보유 자체는 새 법인 것을 쓴다.
PREDECESSOR = {2026053: 1336528,   # 새 CIK → 옛 CIK
               # 2026-09-23 — 47곳 첫 수집에서 두 곳의 이력이 중간에 끊겼다(옛 법인 마지막 분기 2023-06 · 2023-12).
               1998597: 1159159,   # 자나: JANA PARTNERS LLC → JANA Partners Management, LP
...
DROPPED = {
    1336528: ("퍼싱스퀘어 (애크먼) — 옛 법인",
              "2026-06-30 부터 13F-NT(통지)만 내고 보유는 새 법인 PERSHING SQUARE INC."
              "(CIK 2026053)로 간다. 명단을 그쪽으로 옮겼다 — 지운 것이 아니라 갈아탄 것이다."),
    1412093: ("아이칸 캐피털 (아이칸)",
              "장기 미제출 — 최신 보고가 2011-03-31 로 61분기 뒤졌다. 제출을 멈춘 것으로 "
              "보고 2026-08-16 에 명단에서 뺐다. 2026-09-23 — 그 뒤 보유는 개인 명의 "
              "ICAHN CARL C(CIK 921669)로 내고 있어 그쪽으로 되살렸다."),
}
...
def edgar_quarters(cmap: dict):
    ...
    per_cik = {}
    periods = set()
    # 🚨 GURUS 만 돈다. 승계 전 법인을 여기 넣었더니 그 법인의 최신 분기(2026-03-31)가
    #   **전역 기준 분기를 끌어내려 27곳 전부가 한 분기 뒤로 밀렸다**(실측). 승계 문제는
    #   분기 선택이 아니라 «변동 표시» 쪽에서 푼다(아래 _succ 처리).
    for cik in GURUS:
        try:
            fl = H13.filings(cik)
        except Exception:
        ...
        for rd, _fd, acc, _form in sorted(fl):
            # 🚨 2026-08-19 — _form 을 **안 보고 있었다.** 13F 에는 보유를 싣지 않는 서식이
            #   있다: 13F-NT(다른 운용사가 대신 신고했다는 통지)다. 그것까지 «그 분기를 냈다»
            #   로 세면, 실제로는 앞 분기 보유를 들고 있으면서 카드에는 이번 분기 날짜가 찍힌다.
            #   실측(퍼싱스퀘어) — 1336528 이 2026-06-30 에 13F-NT 만 냈는데 화면은
            #   period=2026-06-30 에 11종·137.1억$ 를 실었고, 그 수는 **2026-03-31 신고와
            #   소수점까지 같았다**(실제 06-30 신고는 새 법인 CIK 2026053 의 15종·194.7억$).
            #   → 보유가 실린 서식만 센다.
            if not str(_form or "").startswith("13F-HR"):
                continue
    ...
    # 🚨 여기 도는 것은 **GURUS 뿐이다.** 승계 전 법인을 이 루프에 넣었더니 그 법인의
    #   최신 분기(2026-03-31)가 전역 기준 분기를 끌어내려 **27곳 전부가 한 분기 뒤로
    #   밀렸다**(실측). 승계본은 아래에서 «직전 분기 자리에만» 넣는다.
    for cik in GURUS:
...
def main() -> int:
    ...
    for cik, d in sorted(cur.items(), key=lambda kv: -sum(h["v"] for h in kv[1]["holds"].values())):
        ...
        #   그대로 비교하면 이번 분기 보유가 통째로 «신규» 로 찍힌다 — 애크먼이 새로 산
        #   것이 아니라 법인이 바뀐 것인데 화면이 «13종을 새로 샀다» 고 말하게 된다.
        #   ⚠ 옛 법인을 수집 목록에 넣어 이어 붙이는 방법은 **버렸다** — 그 법인의 최신
        #     분기가 전역 기준 분기를 끌어내려 27곳 전부가 한 분기 뒤로 밀렸다(실측).
        #   그래서 «모른다» 고 적는다. 지어내는 것보다 낫다.
        _succ = cik in PREDECESSOR
        rows = []
        for t, h in d["holds"].items():
            before = p.get(t)
            if before is None:
                chg = "" if _succ else ("신규" if prev_per and prev.get(cik) else "")
            else:
        ...
        managers.append({
            ...
            # 축 — 성적이 아니라 성격이다(정본은 위 AXES 하나)
            "axis": AXIS_OF[cik],
            # 법인 승계로 변동을 못 재는 곳 — 화면이 «신규» 대신 이 사유를 적는다
            "succ_note": (("신고 법인이 바뀌어(옛 CIK %d) 이번 분기 변동을 재지 않습니다 — "
                           "보유는 새 법인 신고 그대로입니다." % PREDECESSOR[cik])
                          if cik in PREDECESSOR else None),
