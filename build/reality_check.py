...
왜 필요한가. 이 랩은 한 창(2016-08~)에서 시행을 450회 돌렸다. `trials.n` 이 그 수를 세어
싣지만, **그 450개의 t 가 전부 잡음이었다면 어떻게 생겼을지**는 아무 데도 안 적혀 있었다.
...
방법 — **계산 전에 여기 적는다**(결과를 보고 고르면 그것도 훑는 것이다).
  ...
  ⑤ 재표집: **원형 블록 부트스트랩**. 블록 길이 **3개월**(= 63거래일, 이 랩이 낙폭
     ...
     🚨 **시간 인덱스를 전 규칙에 «똑같이» 적용한다.** 규칙마다 따로 섞으면 규칙 사이의
       상관이 깨져 독립 시행 450회처럼 보이고, 그러면 잡음의 최고 t 가 과대평가된다.
...
⚠ **이 진단은 450이 아니라 M 개를 덮는다.** 원장·아카이브에 든 시행은 계열이 안 남아 있어
  재표집할 수 없다. M < 450 이면 잡음의 최고 t 가 **과소평가**되고, 그만큼 이 진단은
