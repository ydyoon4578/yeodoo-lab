...
그래서 이 스크립트는 '받아서 덮어쓰기'를 하지 않는다. 아래 관문을 전부 통과해야 쓴다.
  1) 두 소스 합의  : S&P500은 위키백과 목록과 SSGA SPY 보유내역이 겹쳐야 한다(95% 이상).
...
한계 — 숨기지 않고 적는다.
  ...
  · 나스닥100 위키 표의 분류는 GICS가 아니라 ICB다. 그래서 섹터는 S&P500 표(GICS)에서 가져오고,
...
def main() -> int:
    ...
    doc = {
        ...
        "note": "지수 편입 명단. 출처 — S&P500: 위키백과 목록 + SSGA SPY 보유내역 교차 확인, "
