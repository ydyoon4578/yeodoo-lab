...
#    성격이 '위험감축·방어보험·타이밍'인 전략은 CAGR이 아니라 여기서 판단해야 한다.
#    ⚠ 손으로 적으면 조용히 틀린다 — 시계열에서 계산하는 것이 정본이다.
CRISIS = [
    ("gfc2008",  "2008 금융위기",         "2007-10", "2009-02"),
    ("q4_2018",  "2018Q4 긴축 발작",       "2018-09", "2018-12"),
    ("covid",    "2020Q1 코로나",          "2020-01", "2020-03"),
    ("infl2022", "2022 인플레·금리급등",   "2021-12", "2022-12"),
    ("tariff25", "2025 관세 충격",         "2025-01", "2025-04"),
]
...
def vs_block(S, B):
    ...
    # 이 표본이 얼마나 무딘 자인가 — 관측된 Δ를 판정하려면 몇 개월이 필요한가.
    # N_req ≈ N × (최소판정치/|Δ|)². |Δ|가 미세하면 값이 폭주하므로 100년에서 캡한다
    # (55,862년 같은 수는 정보가 아니라 조롱으로 읽힌다).
    if md is not None and abs(d) > 1e-9 and not out["d_sharpe_sig"]:
        nreq = N * (md / abs(d)) ** 2
        out["n_required_months"] = (None if nreq > N_REQ_CAP_MONTHS else int(round(nreq)))
        out["n_required_capped"] = bool(nreq > N_REQ_CAP_MONTHS)
...
# ── 위기 구간 성과 ────────────────────────────────────────────
def crisis_block(months, nav, bnav, b2nav):
    """구간별 전략·대조군 수익(%)과 전략 구간내 낙폭.

    커버 판정은 엄격하다 — from·to 두 월이 **둘 다** 계열 안에 있어야 계산한다.
    부분 커버를 잘라 계산하면 '2008년 −5%' 같은 거짓말이 만들어진다.
    """
    idx = {m: i for i, m in enumerate(months)}
    out = []
    for cid, lab, fm, to in CRISIS:
        row = {"id": cid, "label": lab, "from": fm, "to": to}
        i, j = idx.get(fm), idx.get(to)
        if i is None or j is None or j <= i:
            row["covered"] = False
            row["reason"] = ("이 전략의 계열은 %s ~ %s 라 이 구간을 (온전히) 포함하지 않음"
                             % (months[0], months[-1]))
            out.append(row); continue
        row["covered"] = True

        def seg(a):
            if not a or a[i] in (None, 0): return None
            return _r((a[j] / a[i] - 1) * 100, 1)
        row["s"] = seg(nav); row["b"] = seg(bnav); row["b2"] = seg(b2nav)
        # 구간 안에서 실제로 얼마나 파였는가(구간 시작점을 고점으로 리셋한 낙폭)
        sub = nav[i:j + 1]
        row["s_mdd"] = _r(min(dd_series(sub)) * 100, 1)
        out.append(row)
    return out


def profile_block(m, crisis, given):
    """성격별 1차 지표가 참조하는 보조 수치. **엔진이 못 만드는 값은 null** — 추정하지 않는다."""
    cov = [c for c in crisis if c.get("covered") and c.get("s") is not None]
    worst = min(cov, key=lambda c: c["s"]) if cov else None
    S, B, B2 = m["s"], m["b"], (m.get("b2") or None)
    p = {
        "crisis_covered_n": len(cov),
        "crisis_worst": (worst or {}).get("s"),
        "crisis_worst_id": (worst or {}).get("id"),
        "crisis_worst_label": (worst or {}).get("label"),
        "crisis_worst_bm": (worst or {}).get("b"),
        "d_cagr_vs_bm": (_r(S["cagr"] - B["cagr"]) if (S.get("cagr") is not None
                         and B.get("cagr") is not None) else None),
        "d_mdd_vs_bm": (_r(S["mdd"] - B["mdd"]) if (S.get("mdd") is not None
                        and B.get("mdd") is not None) else None),
        # 타이밍·위험감축 오버레이의 진짜 질문 — '같은 위험을 감수했다면 그냥 덜 사면 됐는가'.
        # bench2_role 이 volmatched 인 계열에서만 그 뜻으로 읽어야 한다(라벨은 화면이 붙인다).
        "d_cagr_vs_bm2": (_r(S["cagr"] - B2["cagr"]) if (B2 and S.get("cagr") is not None
                          and B2.get("cagr") is not None) else None),
        "down_capture": (m.get("vs") or {}).get("down_capture"),
        "exposure_pct": None, "exposure_note": None, "carry_cost": None,
    }
    # 계열 제작 스크립트가 미리 넣어준 값만 통과시킨다(NAV만으로는 산출 불가한 것들)
    for k in ("exposure_pct", "exposure_note", "carry_cost"):
        if given and given.get(k) is not None: p[k] = given[k]
    return p


def identical_prefix(nav, bnav):
    """대조군과 수치상 완전히 같은 선두 구간의 길이(개월).

    유니버스 확장처럼 **정의상** 앞 구간이 모전략과 동일한 계열이 있다. 이 값을 화면에
    적어두지 않으면 겹쳐 그려진 두 선이 차트 버그로 읽힌다(직전 라운드에 실제로 지적됨)."""
    n = 0
    for i in range(len(nav)):
        if bnav is None or i >= len(bnav) or bnav[i] is None or nav[i] is None: break
        if abs(nav[i] - bnav[i]) > 1e-6: break
        n += 1
    return n


def yearly_from_monthly(months, nav, bnav, b2nav):
    """연도별 수익(%) — 월말 NAV에서만 계산한다(차트용 계열).

    첫 해는 기준점(첫 월말) 이후 구간만 담기므로 부분 연도다. 기존 6건은 일별 원자료로
    구워진 값이 이미 있어 덮지 않는다(같은 숫자를 두 벌 만들지 않는다는 규약)."""
    out, seen = [], []
    for i, m in enumerate(months):
        y = m[:4]
        if not seen or seen[-1][0] != y: seen.append((y, i, i))
        else: seen[-1] = (y, seen[-1][1], i)
    for k, (y, first, last) in enumerate(seen):
        base = first - 1 if first > 0 else 0
        if base == last: continue

        def g(a):
            if not a or a[base] in (None, 0) or a[last] is None: return None
            return _r((a[last] / a[base] - 1) * 100, 1)
        out.append({"y": int(y), "r": g(nav), "b": g(bnav), "b2": g(b2nav)})
    return out
...
def compute_strategy(bt, rf_m):
    ...
    # ── 위기 구간 · 성격별 보조 수치 · 대조군과 동일한 선두 구간 ──────────────
    m["crisis"] = crisis_block(mo, nav[:cut + 1], bt["bench"][:cut + 1],
                               (bt.get("bench2") or [None] * (cut + 1))[:cut + 1] if bt.get("bench2") else None)
    m["profile"] = profile_block(m, m["crisis"], bt.get("profile_in"))
    _ip = identical_prefix(nav[:cut + 1], bt["bench"][:cut + 1])
    m["overlap"] = {
        "identical_with_bm_months": _ip,
        "identical_until": (mo[_ip - 1] if _ip > 0 else None),
        # effective_from 이 명시된 계열은 그 앞 구간이 '정의상' 대조군과 같다 → 유효 표본을 따로 센다
        "effective_from": bt.get("effective_from"),
        "effective_n_months": (sum(1 for x in mo if x >= bt["effective_from"])
                               if bt.get("effective_from") else None),
    }
    if m["overlap"]["effective_n_months"] is not None:
...
def main():
    ...
    for name, b in S.items():
        ...
        # 연도별 막대는 차트용 계열이다. 기존 항목은 일별 원자료로 구워진 값이 이미 있어 덮지 않고,
        # 새로 편입된 계열(월말 NAV만 존재)만 여기서 굽는다 — 같은 숫자를 두 벌 만들지 않는다.
        if not b.get("yearly"):
            b["yearly"] = yearly_from_monthly(b["dates"], b["nav"], b["bench"], b.get("bench2"))
            b["yearly_basis"] = "monthly_nav"
        rows.append((name, m))
        pend.append((name, (m["vs"] or {}).get("d_sharpe_p"), m["basis"]["n_months"]))

    # 게시 전략이 6건 → 14건으로 늘면 분모(n_tests)가 함께 늘어 **기존 6건의 문턱도 이동한다**.
    # 이건 정상이다(검정을 더 했으니까). 다만 화면 숫자가 조용히 바뀌면 랩 신뢰가 깨지므로 이력을 남긴다.
    mult = apply_multiplicity(
        pend, source="사이트 게시 전략 일괄 비교 (벤치1 대비 ΔSharpe · m=%d, 2026-07 · 종전 m=6)" % len(
            [x for x in pend if x[1] is not None]))
    for name, m in rows:
