...
def _intraday_block(as_of, adates, apx, sec_etf, sty_etf):
    ...
    def put(bucket, key, nm):
        ...
        # 🚨 **끝점은 표와 같은 수를 쓴다.** 그림의 끝점은 분봉 마지막 체결가인데 표는
        #   공식 종가다 — 종가는 마감 동시호가에서 정해지므로 마지막 분봉과 같지 않다.
        #   실측 2026-08-27: S&P 500 표 0.660 vs 그림 0.647 · 다우존스 30 0.190 vs 0.176.
        #   소수 둘째 자리에서 갈려 **화면에 서로 다른 두 수가 뜬다** — 이 파일이 위에서
        #   배당락을 두고 경계한 바로 그 사고이고, 잔차만 막혀 있지 않았을 뿐이다.
        #   하루 수익의 정본은 공식 종가다(이 랩의 다른 모든 곳이 그 계열을 쓴다).
        #   분봉은 하루 «안의 모양» 을 그리는 재료이지 하루의 «값» 을 정하는 자료가 아니다.
        # ⚠ gaps 는 **못 박기 전** 값으로 잰다 — 아래 가드(mx > 0.15)가 «분봉이 딴
        #   세션이거나 깨졌다» 를 잡는 장치라, 고쳐 놓은 뒤 재면 영영 0 이 되어 죽는다.
        if a and ai is not None and ai > 0 and a[ai] and a[ai - 1]:
            _tab = (a[ai] / a[ai - 1] - 1) * 100
            gaps.append(abs(v[-1] - _tab))
            # 🚨 표와 **같은 자릿수**로 못 박는다. 3자리로 두면 이중 반올림이 된다 —
            #   0.18504 를 여기서 0.185 로 접고 화면이 다시 2자리로 접으면 0.18 인데,
            #   표는 원값을 한 번에 접어 0.19 다. 같은 수를 두 자리로 접었는데 서로
            #   다른 수가 나온다(실측 2026-08-28 DIA). market_board 가 round(...,2) 를
            #   쓰므로 여기서도 2자리다.
            v[-1] = round(_tab, 2)
        bucket[nm] = v
