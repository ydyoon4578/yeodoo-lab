...
PRICE_SIDS = [
             # 2026-09-22 PXSTAT — PREREG-2026-09-22-PXSTAT.md §2. **등록과 함께 미리** 넣는다.
             #   x-volcv 는 거래량+종가, x-dalpha 는 종가만 쓴다. 편출 종목 거래량은
             #   _pit_vol_cache.json 이 덮으므로 x-amihud 가 막혔던 벽이 여기엔 없다.
             #   🚨 이 둘을 넣는 이유가 곧 이 배치의 요지다 — F2(소급÷PIT ≥ 2.0)가
             #     사망 조건이고, PIT 없이는 그 조건을 잴 수가 없다.
             "x-volcv", "x-dalpha",
...
FUND_SIDS = [
             ...
             # 2026-09-05 DEMEGA(카드 A16) — 점수가 시가총액(= 주식수 × 가격)이라
             #   주식수(SEC sh)를 읽는다. 그래서 가격 갈래가 아니라 **여기**다.
             #   선택은 TB.xsec_pick_at 의 x-demega10 갈래를 그대로 탄다(두 레그가 안 갈린다).
             # ⚠ 편출 종목의 주식수 커버가 얇으면 PIT 후보가 랩보다 작아진다 —
             #   그 사실을 결과 문서에 커버리지로 적는다.
             "x-demega10",
             # 2026-09-07 CGATE(카드 A15) — 점수가 시가총액이라 펀더멘털 갈래다.
             #   선택·비중은 TB.xsec_pick_at 의 _cgate_pick 을 그대로 탄다(두 레그가 안 갈린다).
             # ⚠ 그 함수는 **확장 윈도우 이력**을 들고 있다. i 가 되돌아가면 스스로 비우므로
             #   소급 레그와 PIT 레그가 서로의 관측을 섞지 않는다(그쪽 주석 참조).
             "x-cgate", "x-cgate-mom"]
...
# 🚨 후보 «천장» 이 랩의 후보 «바닥» 과 같아 구조적으로 못 도는 규칙 — 2026-09-07.
#   x-ratehot 은 |금리베타| 상위 TOPN*3 종으로 후보를 좁히는데 XSEC_MIN_POOL 도 3*TOPN 이다.
#   두 수가 **같은 식**이라 베타 결측이 하나만 있어도 바닥 아래로 떨어진다 — 실제로
#   121개 리밸 중 120개가 빈 바스켓이었고(후보 중앙값 24종), 남은 한 달로 만든 직선
#   (nav 483점 중 서로 다른 값 5개)에 CAGR·샤프·t 가 붙어 나갔다.
#   ⚠ 이름만 적어 두면 상수가 바뀔 때 낡는다(되풀이 결함 ②). **충돌이 아직 성립하는지
#     여기서 직접 확인**하고, 안 성립하면 특별 문구를 안 쓴다.
_CEIL_EQ_FLOOR = ({"x-ratehot"} if TB.XSEC_MIN_POOL == TB.TOPN * 3 else set())


def _BASE(sid):
    return TB._BASE_SID(sid)
...
def main():
    ...
    def members_at(i):
        ...
        # ⚠ 지역 임포트다. 이 파일의 `import index_members as _IM` 은 **453행의 다른 함수
        #   안**이라 여기서는 안 보인다. 2026-09-04 에 그것을 모르고 _IM 을 써서
        #   **pit_backtest 가 통째로 죽었다**(NameError). 이 파일은 CI 에서 안 도는
        #   로컬 전용이라(가격 캐시가 커밋 금지) **아무도 못 잡았고**, 다음 날 A16 을
        #   돌리려다 드러났다 — 「굽는 잡이 없는 산출물」의 사각지대가 그대로 나왔다.
        import index_members as _IMx                  # noqa: E402  같은 build/ 안
        return _IMx.at(mem, dates[i][:7], label="지수 멤버십")
    ...
    for sid in [s for s in PRICE_SIDS + NEW_PRICE_SIDS + FUND_SIDS + NEW_FUND_SIDS
        ...
        if _p["first"] is None or _b["first"] is None:
            ...
            # 🚨 2026-09-07 — 종전에 여기 「입력 채널이 비었거나 … 못 넘겼다」고 **둘 중
            #   어느 쪽인지 모른 채** 적었다. x-ratehot 을 파 보니 둘째였고, 그것도
            #   우연이 아니라 **구조적 충돌**이었다(아래). 모르는 것을 「거나」로 적으면
            #   화면이 틀린 원인을 지목한다 — 가를 수 있으면 갈라서 적는다.
            _why = ("후보가 커버리지 게이트(%d종)를 한 번도 못 넘겼다" % TB.XSEC_MIN_POOL)
            if _BASE(sid) in _CEIL_EQ_FLOOR:
                _why = ("규칙의 후보 천장이 랩의 후보 바닥과 **같다** — 이 규칙은 |베타| 상위 "
                        "3N 종으로 좁히는데 XSEC_MIN_POOL 도 3N 이다. 결측이 하나라도 있으면 "
                        "바로 바닥 아래로 떨어진다(실측 후보 중앙값 24종). 잴 수 없는 규칙이지 "
                        "성적이 나쁜 규칙이 아니다")
            _nohold[sid] = ("%s 레그가 전 구간 무보유 — %s. 성과가 나쁜 것이 아니라 **안 돈 것**이라 "
                            "수치를 싣지 않는다.%s"
                            % (_leg, _why, (" " + _hl_warn) if _hl_warn else ""))
    ...
    if _nohold:
        ...
        # ⚠ 종전에 여기 「입력 채널 부재」라고 **단정**했는데 x-ratehot 은 그것이 아니었다
        #   (후보 천장 = 랩 바닥). 사유는 규칙마다 다르므로 여기서 하나로 못 박지 않고
        #   excluded 의 개별 사유를 가리킨다.
        _cov_warn.append("전 구간 무보유로 뺀 규칙 %d종(%s) — 사유는 excluded 에 규칙별로 적었다"
