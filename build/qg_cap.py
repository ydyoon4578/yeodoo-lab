...
# 🚨 2026-09-21 사용자 결정 — **편도 25bp → 편도 10bp**(살 때 10 · 팔 때 10).
#   ⚠ 이 변경은 **성적을 좋게 만든다.** 그래서 사유를 적는다:
#   ① 이 값은 처음부터 «가정» 이었고 명세가 없었다(수수료·스프레드·시장충격의 뭉친 값).
#   ② S&P 500 대형주 편도 25bp 는 기관 실측보다 **보수적**이다. 10bp 가 현실에 가깝다.
#   ⚠ 그래도 남는 한계 — **고정률은 규모를 모른다.** 이 전략은 한 종목에 20% 를 담아
#     규모가 커지면 시장충격이 스프레드보다 훨씬 커진다. 10bp 든 25bp 든 그것을 반영하지
#     않는다. **작은 자금이면 보수적, 큰 자금이면 낙관적인 가정이다.**
#   🚨 그리고 이 상수는 «편도» 다 — trade = Σ|Δw| 가 매수·매도를 각각 세므로
#     여기에 곱하면 **한 번 사고 한 번 파는 데 합쳐 20bp** 가 된다.
#     ⚠ 종전 문서가 이것을 «왕복 25bp» 라고 적었는데 **틀렸다.** 편도 25bp 였다.
COST_SIDE = 0.0010
