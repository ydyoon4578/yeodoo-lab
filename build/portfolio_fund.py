...
무엇을 만드나. 실펀드 2종(2Z30=나스닥100 · 2A81=S&P500)의
  ...
  ④ 전략 성과·기여 — BM(전략 첫 매매일부터 지수 등락률) 대비 초과손익과 NAV 기여(bp)
...
정의(화면 각주와 같아야 한다 — 렌더가 이 문서를 그대로 옮긴다):
  ...
  · BM(%) = 그 전략의 첫 매매일부터 기준일까지 지수 등락률. BM 손익 = 매수원금 × BM(%).
    초과 = 손익 − BM 손익(2026-09-22 사용자 지시 — 종전 «매매 시점 일치» 가중합은 첫 매매일이
    같아도 교체매매·매도량이 다르면 BM 이 갈려 «같은 날 시작했는데 왜 다르냐» 가 나왔다).
...
def strat_perf(trades, px, lvl_idx, asof_us):
    """전략 → {curve, last, rows, warn_split}. BM(%) = **전략 첫 매매일 → 기준일 지수 등락률**.

    손익 = Σ qty×(px_t − 체결가). 매도(qty<0)도 같은 식이 성립한다 — 매도 시점에
    잠근 손익이 이후 가격변동과 상쇄되어 실현분으로 남는다.
    BM 손익 = 매수원금(inv) × BM(%) · 초과 = 손익 − BM 손익 = 매수원금 × (수익률 − BM%).

    🚨 2026-09-22 사용자 지시 두 번:
      ① «가중평균 하지말고 전략별로 첫 매매 기준으로» — 매매마다 그 날짜를 0점으로 잡던
         것을 전략 첫 매매일(t0) 하나로. 같은 6/24 개시인데 BM 이 +0.94%/+1.08% 로 갈렸다.
      ② «나스닥BM 그냥 6/24부터 등락률 계산해서 쓰면됨» — ①만으로는 여전히 갈렸다.
         qty×체결가×(지수 등락)을 매매마다 더하면 **매도가 음수로 들어가** BM(%)이
         «지수 등락률 × 순매수/총매수» 가 된다 — 리밸런싱 매도량이 다른 두 전략은 다르다.
         이제 BM(%)은 전략마다 지수 한 줄의 등락률 그 자체이고, 금액은 매수원금에 곱한다.
      ⚠ js/portfolio_app.js 의 calcPerf() 가 같은 정의를 미러링한다 — 같이 고칠 것
        (crossCheck 가 두 엔진을 대조하므로 한쪽만 고치면 화면에 불일치로 뜬다).
    """
    out = {}
    ...
    for _sname, s in out.items():
        ...
        for d in [x for x in dates if x >= t0]:
            ...
            bm = inv * (i_t / i0_fixed - 1) if (i_t and i0_fixed) else 0.0   # 매수원금 × t0→d 지수 등락률
        ...
        for tk, trs in sorted(s["tickers"].items()):
            ...
            bm_t = inv_t * (i_t / i0_fixed - 1) if (i_t and i0_fixed) else 0.0   # curve 와 같은 정의
...
def render_fund(fund, idx, slug, label, nav, fx, hold, cons, trades, px, lvl, axis):
    ...
    H.append('<tr class="totrow">'
             ...
             + _tot(_sws, tip="전략이 든 %d종에서 펀드비중 합 %s − 지수비중 합 %s = %+.2f%%p. "
                              "전략 합계에서 지수를 직접 빼지 않는다 — 전략은 패시브 위에 "
                              "얹는 틸트다. (게시 시점 기준. 웹 원장을 고치면 이 설명은 낡는다)"
                              % (len(_srows), pct(_swf_s, 2), pct(_swi_s, 2), _ds))
    ...
    H.append('<h3>④ 전략 성과·기여 <span class="hnote">라이브 계산 · BM = 전략 첫 매매일부터 지수 등락률</span></h3>')
...
<li><b>BM</b>은 그 전략의 <b>첫 매매일부터 기준일까지 지수 등락률</b>이다(전략마다 지수 한 줄 — 매매가 여러 건이어도, 도중에 팔았어도 같다). BM 손익 = 매수원금 × BM, 초과 = 전략 손익 − BM 손익. <b>NAV 기여(bp)</b> = 초과(USD) × 기준일 환율 ÷ NAV × 10,000.</li>
