...
# 🚨 2026-09-07 사용자 결정 — **「10·20·30 격자」를 N 의 근거로 쓰지 않는다.**
#   2026-08-29 에 «셋 다 돌려 좋은 것을 고르기»(nsel)는 없앴지만, 그 뒤에도
#   «셋 다 싣고 안 고른다» 를 **N 을 정하는 근거**로 쓴 등록이 있었다
#   (PREREG-2026-08-29-RESIDIND §1 → x-residind 의 N=10). 그것도 이제 안 한다.
#   ⚠ 새 규칙의 N 은 **원 문헌에서 끌어온다.** 원문이 분위로 말하면 그 분위를 이 유니버스로
#     옮긴다(십분위 = 518÷10 = 52 — 랩에 이미 6종의 선례가 있다).
#   ⚠ **원문이 N 을 안 주면 N 을 만들지 않는다.** 그런 카드는 `pool_triage` 의 `underspec`
#     으로 남긴다 — 랩 기본값 TOPN 으로 채우는 것도 «내가 정한 수» 다.
#   규약: PREREG-2026-09-07-RESIDIND2.md §1.
#
# 🚨 2026-08-29 — NSWEEP_GRID(10·20·30)와 변형 표식 `~n` 을 **없앴다**
#   (PREREG-2026-08-29-ASWRITTEN.md §0 · 사용자 지시). 바스켓 크기를 셋 돌려 샤프가
#   가장 높은 것을 고르던 절차가 사라졌으므로, 그 절차만 쓰던 이 두 상수도 같이 나간다.
#   ⚠ 여기 붙어 있던 경고(«이 글자를 모르는 코드가 변형을 다른 규칙으로 착각한다 —
#     은퇴 필터가 `sid not in RETIRED` 로 검사해 은퇴한 10종이 변형을 통해 되살아나
#     있었다», 2026-08-13 실측)는 이제 가리킬 변형이 없다. 다만 **접미사 있는 sid 는
#     남아 있다**(`-n52` 원 전략 크기 6종 · `-band` 저회전 변형) — sid 를 다루는 자리는
#     계속 `_BASE_SID()` 로 밑동을 먼저 구할 것. 그 교훈은 표식이 바뀌어도 그대로다.
...
SCREEN_SIDS: set = {"x-cgate", "x-cgate-mom"}   # 정의는 둘 다 남기고, 등록은 mom 만 살아 있다
...
#   🚨 2026-08-29 — 이 명단을 쓰는 곳이 **하나로 줄었다.** 종전에는 둘이었다(바스켓 크기
#     전수 시험 NSW_SKIP 에서 빼는 자리 + 산출물 bask.tgt 을 None 으로 내보내는 자리).
#     전수 시험이 폐기돼(PREREG-2026-08-29-ASWRITTEN.md §0) 앞의 것이 사라졌다.
#   ⚠ 그래도 명단은 **한 벌로 둔다.** 소비처가 다시 늘 때 두 벌로 적으면 반드시 갈린다.
...
# 🚨 «직전 보유를 알아야 하는 규칙» 의 명단. 접미사(-band)로 걸리는 것 말고, 규칙 자체에
#   이력현상이 들어 있는 것이 2026-08-29 부터 하나 있다(x-btp-n155 — E3 원문이 히스테리시스
#   밴드를 지정한다. PREREG-2026-08-29-ASWRITTEN.md §2).
# ⚠ 명단을 늘릴 때는 **선택이 경로의존이 된다**는 것을 알고 늘릴 것. 같은 격자를 다시 돌려도
#   시작점이 다르면 다른 바스켓이 나온다 — 밴드가 원래 그런 규칙이고, 그래서 명단으로 둔다.
HELD_SIDS = {"x-btp-n155"}


def needs_held(sid):
    return is_band(sid) or sid in HELD_SIDS
...
def june_ends(dates):
    """연 1회 리밸 인덱스 — **t년 6월말**. Fama-French 컨벤션.

    🚨 6월인 것에 이유가 있다. 전략 탐색 풀 E8(자산성장 아노말리)의 entry 가 그대로 적어
      두었다: "리밸런싱: 연 1회, 회계연도 종료 후 재무제표 공시 지연을 고려해 **t년 6월
      시점** 포트폴리오 구성(Fama-French 컨벤션)." 12월 결산 기업의 전년도 재무제표가
      그때쯤 전부 공개돼 있다는 뜻이고, 이 랩의 FUND_LAG_DAYS 와 목적이 같다
      (다만 이쪽은 **날짜를 못박아** 종목마다 다른 지연을 한 시점으로 모은다).
    ⚠ 6월 말일이 아니라 **6월의 마지막 거래일**이다 — 이 랩의 다른 주기와 같은 규약.
    ⚠ 이 주기를 쓰는 규칙은 표본에서 리밸 횟수가 한 자릿수다(10년 격자 → 최대 10회).
      회전율·t 를 읽을 때 그 사실을 같이 볼 것.
    """
    return [i for i in month_ends(dates) if dates[i][5:7] == "06"]
...
#   셋을 다 돌려 좋은 것을 고르면 그게 또 하나의 선택 부풀림이 된다. 바스켓 크기(NSWEEP)는
#   사용자 결정으로 한 번 그렇게 했다가 2026-08-29 에 폐기됐다 — 이쪽은 애초에 안 탔다.
REB_KINDS = {"we": "주간", "me": "월말", "qe": "분기", "y6": "연 1회(6월말)"}
REB_DEFAULT = "me"
...
def reb_index(kind, dates):
    ...
    return {"we": week_ends, "me": month_ends, "qe": quarter_ends,
            "y6": june_ends}[kind](dates)
...
def pick_neutral_2ax(sc, sec_of, mcap_of, n, sid="", held=None, band=None):
    """섹터 × 시총 5분위 **두 축 중립** 바스켓 — E3 원문의 「시가총액·섹터 중립화」.

    전략 탐색 풀 E3(밸류 HML)의 entry 를 그대로 옮긴다:
      "PIT 유니버스를 B/M … 으로 정렬, 상위 30% 롱 … **시가총액·섹터 중립화 권장**.
       월말~연 1회 리밸런싱, 회전율 관리를 위해 **히스테리시스 밴드 적용**."

    🚨 「중립화」를 「시총가중」으로 읽지 않는다(사용자 결정 2026-08-29). 같은 탐색 풀의
      E8 카드가 시총가중을 **「가치가중」**이라고 부른다("동일가중과 가치가중 모두 검증") —
      이 풀은 두 단어를 구별해 쓰고, E3 은 「중립화」라고 적었다. 그래서 중립화로 읽는다.
      ⚠ 첫 구현은 시총가중이었고 MSFT 한 종목이 17~20% 를 먹었다. 155종을 담고도 사실상
        대형 성장주 한둘이 성적을 정하는 바스켓이었다 — 그것은 「중립화」의 반대다.

    무엇을 중립화하나 — **두 축을 같은 잣대로** 잰다.
      · 섹터 축: 포트폴리오의 섹터별 종목 비중 = 유니버스 후보의 섹터별 종목 비중
      · 시총 축: 시총 5분위별 종목 비중도 마찬가지
    즉 (섹터 × 5분위) 칸마다 후보 수에 비례해 자리를 나누고, 칸 안에서는 점수 상위를
    **동일가중**으로 담는다. 그러면 두 분포가 유니버스와 같아진다.
    🚨 한 축은 시총 비중, 다른 축은 종목 비중으로 재면 **두 축을 서로 다른 벤치마크에
      맞추는 것**이라 중립이 성립하지 않는다. 이 랩의 매매 대상 대조군이 동일가중
      유니버스(vs_traded)이므로 둘 다 종목 비중으로 잰다.
    ⚠ 그 결과 이 규칙의 비중은 **동일가중**이다(칸 비중 ÷ 칸 자리 수 = 1/n 로 상쇄된다).
      «가중을 안 바꿨다» 가 아니라 «두 축을 중립화하면 동일가중이 나온다» 가 맞는 설명이다.
      바뀐 것은 비중이 아니라 **무엇을 고르느냐**다.

    ⚠ 5분위 나누는 법은 새로 정하지 않는다 — 이 랩에 이미 등록된 F3 규약을 그대로 쓴다
      (PREREG-2026-08-14-FLOW.md §4: "그달 후보의 20/40/60/80 백분위 · 각 분위에서 N/5,
      나머지는 큰 분위부터"). 같은 것을 두 벌로 정의하면 한쪽만 고쳐지는 날이 온다.

    인자
      sc        점수 내림차순 [(점수, 티커)…]
      sec_of    티커 → GICS 섹터. 빈 문자열이면 **뺀다**(pick_sector_neutral 과 같은 규약 —
                '기타' 칸을 만들면 그 칸이 중립의 예외가 되어 중립을 확인할 수 없다).
      mcap_of   티커 → 시가총액. **없으면 뺀다** — 시총을 모르면 어느 분위인지 모르고,
                모르는 것을 한 칸에 몰아 넣으면 그 칸이 다시 중립의 예외가 된다.
      n         전체 목표 종목 수(이 규칙은 155)
      held      직전 보유. band 가 없으면 안 쓴다.
      band      파는 문턱 배수. 칸 안에서 상위 band×k 밖으로 나갈 때만 판다.

    돌려주는 것 — 티커 리스트(동일가중이므로 비중을 따로 안 준다).
    """
    rows = [(v, t) for v, t in sc
            if (sec_of.get(t) or "").strip() and (mcap_of.get(t) or 0) > 0]
    if not rows:
        return []
    # ── 시총 5분위 — F3 규약(그달 후보의 20/40/60/80 백분위) ────────────────
    vals = sorted(mcap_of[t] for _v, t in rows)
    cut = [vals[int(len(vals) * q)] for q in (0.2, 0.4, 0.6, 0.8)]

    def _q(t):
        return sum(1 for c in cut if mcap_of[t] > c)

    by = {}
    for v, t in rows:                              # rows 는 점수 내림차순
        by.setdefault(((sec_of.get(t) or "").strip(), _q(t)), []).append((v, t))
    # ── n 자리를 칸에 나눈다 — 후보 수에 비례, 최대잔여법, 칸마다 최소 1 ────────
    # ⚠ 배분 규칙을 여기서 **하나만** 쓴다. 결과를 보고 바꾸면 그것이 곧 최적화다.
    # ⚠ 「최소 1」은 중립이 요구하는 것이다 — 자리가 0 인 칸은 그 칸의 비중이 통째로
    #   사라져 중립이 깨진다. 대신 그만큼 큰 칸이 자기 몫보다 조금 적게 받는다.
    #   칸이 최대 55개(11섹터 × 5분위)이고 n=155 라 이 왜곡은 작지만 0 은 아니다.
    cand = {g: len(v) for g, v in by.items()}
    csum = float(sum(cand.values()))
    raw = {g: n * cand[g] / csum for g in by}
    k = {g: max(1, min(cand[g], int(raw[g]))) for g in by}
    rem = n - sum(k.values())
    if rem > 0:                                    # 남은 자리 — 잔여가 큰 칸부터
        order = sorted(by, key=lambda g: (-(raw[g] - int(raw[g])), str(g)))
        j = 0
        while rem > 0 and any(k[g] < cand[g] for g in order):
            g = order[j % len(order)]
            if k[g] < cand[g]:
                k[g] += 1
                rem -= 1
            j += 1
    elif rem < 0:                                  # 최소 1 로 넘친 몫 — 큰 칸부터 되돌린다
        order = sorted(by, key=lambda g: (-k[g], str(g)))
        j = 0
        while rem < 0 and any(k[g] > 1 for g in order):
            g = order[j % len(order)]
            if k[g] > 1:
                k[g] -= 1
                rem += 1
            j += 1
    # ── 칸 안에서 고른다 — 이중클래스 규약과 밴드는 pick_top/pick_band 를 그대로 쓴다 ──
    qof = {t: _q(t) for _v, t in rows}
    out = []
    for g, rs in by.items():
        ...
        if kg <= 0:
            continue
        if band:
            hg = [t for t in (held or [])
                  if ((sec_of.get(t) or "").strip(), qof.get(t)) == g]
            out += pick_band(rs, sid, kg, hg)
        else:
            out += pick_top(rs, sid, kg)
    return out
...
def _BASE_SID(sid):
    ...
    ⚠ 셋째였던 `~n<숫자>`(2026-08-13 전수 시험)는 2026-08-29 에 폐기됐다
      (PREREG-2026-08-29-ASWRITTEN.md §0). 정규식에서는 `~` 를 뺐다.
...
def build_strats():
    ...
    # 🚨 12-1 점수는 **한 곳에서만** 만든다. x-mom12-n52(QUANTILE4)가 같은 점수를 써야
    #   하는데 람다를 복사해 넘기면 그 순간 사본이 둘이 되고, 이 저장소가 되풀이 밟은
    #   «한쪽만 고쳐지는 날» 이 온다(secrev_panel 머리말의 같은 규약).
    _mom12_fn = (lambda t, i, P, R, V: ((lambda a: None if a is None
...
#         "거래량과 매일 쌓인 거래량은 다른 것이라는 가정. 둘 다 통과하면 그 자체가 정보다.")
    xsec("x-guruacc", "거장 순매집 상위 %d" % TOPN,
         "13F 공시 거장 22~27인(분기마다 다르다 · 오늘 27인)의 합산 보유금액이 직전 분기 대비 가장 크게 늘어난 %d종목 동일가중, "
         "월말 리밸런스. 분기말 +%d일(공시 마감) 이후 첫 월말부터 쓴다." % (TOPN, F13_LAG),
         None,
         "t-13f-clone 은 보유 수준(많이 든 것)을 복제하고 이건 변화(더 담은 것)를 본다. "
         "🚨 이름에 '기관'을 쓰지 않았다 — 이들은 기관 수급이 아니라 거장 포지셔닝이고, "
         "이름이 자료보다 크면 그 자체가 거짓말이다. "
         "⚠ 보유 매니저 수는 순위로 쓰지 않는다. 스물 몇 명뿐이라 서로 다른 값이 얇다"
    ...
    # ── 원 전략 6종 — 「그대로」 재현 ─────────────────────────────────
    # 사전등록 ① PREREG-2026-08-11-BASKET.md (2026-08-11 커밋 ba8080df) — **N 만** 맞췄다.
    #           ② PREREG-2026-08-29-ASWRITTEN.md — ①이 「안 따른 것」 열에 적어 둔 항목을
    #              구현한다(사용자 지시 「전략이 말하는 그대로 하라」·「싹다 원문대로 맞추라」).
    # 🚨 ①은 «N 만 바꾼다» 를 규약으로 삼고 안 따른 것을 표에 적었다("안 적으면 '원본대로
    #   했다'는 거짓말이 된다"). ②는 그 표를 실행한다 — 그러므로 여기서 «점수 함수는 같고
    #   크기만 다르다» 가 더는 여섯 다에 해당하지 않는다. 규칙마다 아래에 무엇을 맞췄고
    #   무엇을 못 맞췄는지 적는다. 뭉뚱그리지 않는 것이 ② §3 의 요구다.
    # ⚠ 원문 문구는 전략 탐색 풀(data/rotation_pool.json)의 entry 에서 **그대로** 인용한다.
    #   근거 없이 문턱을 정하지 않는다.
    _BSK = ("이 규칙은 짝이 되는 10종판과 점수 함수가 같다. 크기는 전략 탐색 풀의 원문에서 "
            "왔다 — 이 랩이 고른 값이 아니다. "
            "⚠ 증분알파(incr5)의 최근접 이웃은 그 10종판으로 나올 것이 거의 확실하다. "
            "같은 점수를 쓰므로 서로를 설명한다 — 판정은 단독 t 와 부호로 읽을 것.")
    # 원문대로 맞춘 규칙에 붙이는 꼬리표. ⚠ **구현이 끝난 것에만** 붙인다(등록 ② §3).
    _ASW = (" 🚨 2026-08-29 — 이 규칙은 원문의 크기뿐 아니라 아래 항목까지 맞췄다. "
            "그래서 짝인 10종판과 «크기만 다른 같은 규칙» 이 아니다 — 둘의 차이를 "
            "크기 탓으로 읽으면 안 된다.")
    xsec("x-lowvol-n100", "저변동성 상위 100 · 역변동성 가중 (원 전략)",
         "최근 60일 실현변동성이 가장 낮은 100종목을 역변동성 가중(비중 ∝ 1/변동성)으로 "
         "담는다, 월말 리밸런스. 정지시세(직전 252일 중 일간수익 0 인 날이 10% 초과)는 제외.",
         # 위 x-lowvol 과 같은 게이트 — 짝인 두 규칙이 갈리면 비교가 성립하지 않는다.
         lambda t, i, P, R, V: (None if (V is None or _stall(R, i)) else -V),
         "E2 저변동성 아노말리/BAB 의 원문은 '롱온리 변형은 저변동 상위 100종목 역변동성 "
         "가중. 월간 리밸런싱'이다. 이 랩은 그것을 10종 동일가중으로 좁혀 돌리고 있었다. "
         + _BSK + _ASW +
         " 맞춘 것: 크기 100 · 역변동성 가중 · 월간 리밸런스(셋 다 원문 그대로). "
         "⚠ 원문의 롱숏 다리(하위 20% 롱 베타가중 · 상위 20% 숏)는 안 만든다 — "
         "카드가 그것을 '롱온리 변형' 과 나란히 적었고, 이 랩이 재는 것은 롱온리 쪽이다.",
         topn=100)
    xsec("x-maxlow-n52", "복권형 회피 — MAX 최하위 52 (십분위)",
         "최근 1개월 일별 수익의 최댓값이 가장 작은 52종목 동일가중, 월말 리밸런스.",
         lambda t, i, P, R, V: (lambda m: (-m) if m is not None else None)(maxret(R, i, 21, 1)),
         "E18 복권선호 회피의 원문은 '유니버스 내 횡단면 십분위 정렬'이다. 518종의 십분위는 52다. "
         + _BSK +
         " 🚨 이 규칙은 2026-08-29 등록에서 원문대로 못 맞춘 쪽이다. E18 이 권장하는 "
         "'구현 A' 는 «챔피언/모멘텀 후보 바스켓에서 MAX(5) 상위 10~20% 를 사전 배제한 뒤 "
         "동일가중 top-N» 인데, 이것은 바스켓 크기 문제가 아니라 오버레이이고 "
         "배제 비율(10~20%)과 챔피언 집합이 원문에 안 정해져 있다. 내가 그 수를 고르면 "
         "방금 없앤 자유도를 옮기는 것이라 규칙을 안 바꿨다 — 구현 A 는 별도 측정으로만 "
         "재고 결과 문서에 적는다(PREREG-2026-08-29-ASWRITTEN-RESULT.md). "
         "⚠ E18 의 '필수 정제'(베타 중립화 MAXβ · 실적발표일 ±1일 제외 · 신주발행 통제)도 "
         "안 따랐다. 이 판이 재는 것은 여전히 크기 하나다.",
         topn=52)
    xsec("x-max5low-n52", "복권형 회피 — MAX(5) 최하위 52 (십분위)",
         ...
         " 🚨 구현 A(챔피언 바스켓 사전배제형)와 필수 정제는 짝인 MAX(1) 판과 같은 사유로 "
         "안 따랐다 — 위 규칙의 설명을 볼 것.",
         topn=52)
    xsec("x-btp-n155", "장부가 대비 저평가 — 상위 30% · 섹터×시총 중립 · 밴드 (원 전략)",
         "주당순자산 ÷ 주가로 정렬하되, 유니버스를 (GICS 섹터 × 시총 5분위) 칸으로 나눠 "
         "칸마다 후보 수에 비례하는 자리를 주고 그 안에서 상위를 뽑아 155종을 동일가중으로 "
         "담는다. 그러면 바스켓의 섹터 분포와 시총 분포가 유니버스와 같아진다. "
         "회전을 줄이려고 히스테리시스 밴드를 건다 — 칸 안 상위 k 에 들면 사고, "
         "상위 %d×k 밖으로 나갈 때만 판다. 시총을 모르는 종목은 제외. 월말 리밸런스."
         % int(BAND),
         None,
         "E3 밸류(HML)의 원문은 '상위 30% 롱 … 시가총액·섹터 중립화 권장. 월말~연 1회 "
         "리밸런싱, 회전율 관리를 위해 히스테리시스 밴드 적용'이다. 518종의 30%는 155다. "
         + _BSK + _ASW +
         " 맞춘 것: 크기 155 · 시가총액 중립화 · 섹터 중립화 · 히스테리시스 밴드. "
         "🚨 '중립화'를 '시총가중'으로 읽지 않았다. 같은 탐색 풀의 E8 카드가 시총가중을 "
         "'가치가중'이라고 부르므로(이 풀은 두 단어를 구별해 쓴다), E3 의 '중립화'는 "
         "노출을 유니버스에 맞춘다는 뜻으로 읽는다. 사용자 결정 2026-08-29. "
         "⚠ 그 결과 이 규칙의 비중은 동일가중이다 — 두 축을 종목 비중으로 중립화하면 "
         "칸 비중이 칸 자리 수로 나뉘어 상쇄되기 때문이다. 바뀐 것은 비중이 아니라 "
         "무엇을 고르느냐다. "
         "⚠ 밴드 폭은 이 랩에 이미 등록된 값 하나(2.0)를 쓴다 — 카드가 폭을 안 적었고, "
         "여러 값을 돌려 좋은 것을 고르면 그것이 곧 최적화다(PREREG-2026-08-24-LOWTURN). "
         "⚠ 155종은 유니버스의 30%인데다 이제 섹터·시총 분포까지 유니버스와 같다 — "
         "vs_traded(랩 동일가중) 초과가 0 근처면 이것은 밸류가 아니라 지수를 조금 "
         "기울인 것이다. 그 판정을 위해 이 열을 먼저 볼 것.",
         topn=155)
    xsec("x-payout-n50", "총주주환원수익률 상위 50 (원 전략 크기)",
         "12개월 배당과 자사주매입의 합을 시가총액으로 나눈 값이 가장 큰 50종목 동일가중, "
         "월말 리밸런스.",
         None,
         "A1 자사주매입+배당성장의 원문은 '동일가중 40-60종목'이다. 양 끝을 고르면 그것부터가 "
         "자유도이므로 중앙값 50을 쓴다. " + _BSK +
         " 🚨 2026-08-29 — 원문의 자격필터를 재 보고 안 걸었다. 카드의 문턱은 "
         "'Buyback yield > 3% + 5년 배당 CAGR > 5% + 순부채/EBITDA < 3배' 인데, 이 유니버스에서 "
         "셋을 다 통과하는 종목이 한 달도 50종에 못 미친다(실측: 2014~2026 년 초 기준 "
         "11~32종, 2013년 이전은 배당 5년 이력이 없어 0종). 즉 카드의 두 조항 — 자격필터와 "
         "'동일가중 40-60종목' — 이 이 유니버스에서 동시에 성립하지 않는다. "
         "필터를 걸고 남는 것을 전부 담으면 크기가 50이 아니고, 크기를 지키면 필터가 안 걸린다. "
         "둘 중 하나를 내가 고르면 그것이 새 자유도라 규칙을 안 바꿨다 — 측정과 사유는 "
         "결과 문서에 싣는다(PREREG-2026-08-29-ASWRITTEN-RESULT.md). "
         "⚠ 원문의 분기 리밸런스는 이미 따르고 있다(PREREG-2026-08-13-REBAL 의 분기 표). "
         "⚠ 원문의 청산 조건('순발행이 플러스로 전환되면 청산')도 안 걸었다.",
         topn=50)
    # ── 「분위」라고만 적힌 넷 — PREREG-2026-09-02-QUANTILE4.md (계산 전 커밋 555a3f66c) ──
    # 🚨 ASWRITTEN 이 일부러 유예한 넷이다. 그때 «원문에 없는 수를 내가 정하면 방금 없앤
    #   자유도를 옮기는 것» 이라 안 건드렸고, 사용자 결정(2026-09-02)으로 수를 정했다.
    #   근거는 내 취향이 아니라 **각 카드의 원 논문**이다(등록 §1).
    # ⚠ 넷을 하나의 수로 통일하지 않는다 — A20 만 155 다. George·Hwang(2004)이 데실이
    #   아니라 30/40/30 을 쓰기 때문이고, 52 로 맞추면 «보기 좋다» 는 이유로 원문을 어기는 것이다.
    # ⚠ 점수·신호·리밸은 짝과 한 글자도 다르지 않다(_BASE_SID 로 같은 갈래). topn 만 다르다.
    _Q4 = ("이 규칙은 짝이 되는 10종판과 점수 함수가 같다 — 크기만 다르다. 그 크기는 "
           "전략 탐색 풀의 카드가 「분위」라고만 적어 ASWRITTEN 이 유예했던 것이고, "
           "사용자 결정(2026-09-02) 뒤 원 논문의 분위 관례에서 가져왔다. "
           "이 랩이 성적을 보고 고른 값이 아니며, 5분위판을 같이 돌려 나은 쪽을 "
           "취하지도 않았다(그것이 2026-08-29 에 폐기한 nsel 이다). "
           "규약 PREREG-2026-09-02-QUANTILE4.md. "
           "⚠ 증분알파(incr5)의 최근접 이웃은 그 10종판으로 나올 것이 거의 확실하다 — "
           "같은 점수를 쓰므로 서로를 설명한다. 판정은 단독 t 와 부호로 읽을 것.")
    xsec("x-mom12-n52", "12-1 모멘텀 상위 52 (십분위 · 원 전략)",
         "최근 12개월 수익 − 최근 1개월 수익 상위 52종목 동일가중, 월말 리밸런스.",
         # 🚨 fn=None 을 넘기면 안 된다. x-mom12 는 채점 갈래가 아니라 람다로 도는
         #   규칙이라, 형제가 None 이면 _BASE_SID 로 갈래를 찾다가 마지막 else 의
         #   S["fn"](...) 에서 죽는다(2026-09-02 실측: TypeError NoneType not callable).
         #   짝이 갈래형(x-52wh·x-residmom·x-shiss)인 셋은 None 이 맞다 — 그래서 셋만
         #   보고 넷 다 None 으로 뒀다가 걸렸다.
         _mom12_fn,
         "A5 횡단면 모멘텀의 원문은 「상위 분위」라고만 적는다. 그 카드가 인용한 주 논문 "
         "Jegadeesh·Titman(1993, JF)이 십분위 정렬로 최상 데실을 롱하므로 52(=518÷10)를 쓴다. "
         + _Q4 +
         " 🚨 짝인 x-mom12(10종)는 이 랩 여러 규칙의 증분알파 대조군이다 — 그 자리를 "
         "이 규칙이 대신하지 않는다. 대조군을 바꾸면 과거 수치와 비교가 끊긴다.",
         topn=52)
    xsec("x-residmom-n52", "잔차 모멘텀 상위 52 (십분위 · 원 전략)",
         "과거 36개월 월간수익을 시장·규모·가치에 회귀한 잔차의 12-1개월 합을 잔차 "
         "표준편차로 나눠 상위 52종목 동일가중, 월말 리밸런스. 직전 1개월은 건너뛴다.",
         None,
         "B4 잔차 모멘텀의 원문도 「상위 분위」라고만 적는다. 잔차 모멘텀 문헌이 "
         "Jegadeesh·Titman 의 정렬을 그대로 물려받으므로 같은 십분위(52)를 쓴다. " + _Q4,
         topn=52)
    xsec("x-shiss-n52", "순주식발행 회피 — 최저 52 (십분위 · 원 전략)",
         "가중평균 희석주식수의 전년 동기 대비 증가율이 가장 낮은 52종목 동일가중, "
         "월말 리밸런스. 증가율 절대값이 50%를 넘으면 제외.",
         None,
         "E44 순주식발행의 원문은 「유니버스를 분위로 나누고」라고만 적지만, 카드 본문이 "
         "스스로 십분위를 인용한다 — 「복합주식발행 최상·최하 십분위 차의 평균수익은 월 −0.…」. "
         "Fama·French(2008)·Hou·Xue·Zhang(w23394)도 십분위다. 그래서 52 다. " + _Q4 +
         " ⚠ 이 등록은 크기만 만졌다. 카드는 「복합주식발행을 주 지표로 삼는 편이 낫다」고도 "
         "적는데 이 규칙은 주식 수 기반이다 — 신호까지 같이 바꾸면 둘 중 무엇이 성적을 "
         "움직였는지 못 가린다. 복합주식발행은 별도 등록 감이다.",
         topn=52)
    xsec("x-52wh-n155", "52주 신고가 근접 상위 155 (상위 30% · 원 전략)",
         "현재가를 직전 252거래일 일중 고가의 최대로 나눈 값이 가장 큰 155종목 동일가중, "
         "월말 리밸런스. 그 창의 유효 관측이 200일 미만이면 후보에서 제외.",
         None,
         "A20 52주 신고가의 원문도 「상위 분위」라고만 적는다. 🚨 그런데 그 카드가 인용한 주 논문 "
         "George·Hwang(2004, JF)은 데실이 아니라 30/40/30 으로 승자·패자를 가른다 — "
    ...
    # 🚨 2026-09-07 — **모멘텀판만 되올렸다**(PREREG-2026-09-07-CGATEMOM · 커밋 f9c03ec9).
    #   ⚠ **게시가 아니다.** strategy_index.HIDE_SIDS 에 넣어 화면에서 뺀다 —
    #     이 랩의 규약으로 HIDE_SIDS 는 «재면서 화면에서만 뺀다» 이고 RETIRED 는
    #     «사전등록 기각» 이다. 지금 상태는 전자다.
    #   🚨 되올린 목적은 **전방(out-of-sample)** 이다. 소급 t 2.10 은 근거가 아니다 —
    #     같은 창에서 다시 돌리면 같은 수가 나오고, 그것을 재검증이라 부르면 한 번 본 것을
    #     두 번 세는 것이다. 얼린 경계는 **2026-09-04** 이고 그 뒤 구간만 판정에 쓴다.
    #   ⚠ 규칙을 한 글자도 안 바꾼다. 만지면 등록 F5 로 전방 표본이 0 으로 되돌아간다.
    #   ⚠ 수준판(x-cgate)은 되올리지 않는다 — 그쪽은 F2 에서 기각됐다.
    xsec("x-cgate-mom", "지수 집중도 게이트 — 12개월 변화 (전방 검정 중 · 미게시)",
         "그 월말 채점 가능 종목의 시가총액 비중으로 허핀달 집중도를 재고, 그 12개월 "
         "변화의 확장 윈도우 백분위가 상위 1/3 이면 전 종목 동일가중, 하위 1/3 이면 "
         "시가총액가중, 중간이면 두 비중을 절반씩 섞는다. 월말 리밸런스.",
         None,
         "🚨 전방 검정 중이라 게시하지 않는다(PREREG-2026-09-07-CGATEMOM). "
         "A15 를 기각할 때 병행판으로 돌려 소급 t 2.10 이 나왔지만, 그 등록이 계산 전에 "
         "「좋은 쪽을 고르지 않는다」고 못박아 판정을 안 바꿨다. 이 등록은 «그 결과를 보고 "
         "쓴 것» 이고 그래서 가설은 사전이 아니다 — 사전인 것은 «이제부터 어떻게 잴 것인가» "
         "뿐이다. 같은 창에서 다시 돌리는 것은 검정이 아니므로 얼린 경계(2026-09-04) 뒤 "
         "구간만 판정에 쓴다. 전방 24개월이 차기 전에는 어떤 수치도 판정이 아니다. "
         "⚠ 이 랩의 다중검정 진단은 잡음만으로도 최고 t 중앙값이 2.26 이라고 말한다 — "
         "소급 t 2.10 은 그 분포 안쪽이다. 그것이 전방으로 미룬 이유다.",
         topn=999, reb="me")
    ...
    xsec("x-bmrot-flat", "B/M 절반 상시보유 (틸트 없음 · x-bmrot 대조군)",
         ...
         topn=999, reb="me")
    # 🚨 2026-09-03 — **x-a1payout 의 등록을 내렸다.** PREREG-2026-09-03-A1FIX 의 F2
    #   (PIT t ≥ 1.5)에서 t 1.24 로 기각됐고, 판정문이 「게시하지 않는다 … 종전 판의
    #   판정은 철회한다」「x-a1payout 은 게시 목록에서 빠진다」라고 두 번 못박았다.
    #   그 배선이 09-03 까지 아무 데도 안 돼서 게시 48종에 그대로 서 있었다(새 유형
    #   「판정만 하고 안 배선」). 이제 validate_site 가 «기각 원장에 실린 규칙이 게시
    #   목록에 그대로 있다» 로 잡는다.
    # ⚠ 이 랩의 규약은 «원장에 실린 종목 규칙은 엔진에 없다» 다 — 실측으로 원장의
    #   종목 sid 117개 중 114개가 등록에도 퇴출에도 없다(나머지 셋은 readmitted 둘 +
    #   이 규칙). RETIRED 에 넣으면 «퇴출 목록과 세 번째 목록에 둘 다 있다» 로 걸린다.
    # ⚠ 정의·측정은 사라지지 않는다 — 규약과 수치는 PREREG-2026-09-02-A1PAYOUT.md ·
    #   PREREG-2026-09-03-A1FIX(-RESULT).md 와 build/tested_not_published.json 에 있다.
    #   기전: 분자(자사주매입+배당)와 분모(시가총액)의 시점이 어긋나 있었고 맞추자
    #   우위가 사라졌다. 「나쁜 전략」이 아니라 «옛 우위가 시점 불일치였다» 는 결과다.
    xsec("x-agrow-n52", "자산성장 회피 — 최저 52 · 연 1회 6월 리밸 (원 전략)",
         "직전 공개 회계연도 총자산 증가율이 가장 낮은 52종목 동일가중, 월말 리밸런스.",
         None,
         "E8 자산성장 아노말리의 원문은 '하위 십분위 롱 … 리밸런싱: 연 1회, 회계연도 종료 후 "
         "재무제표 공시 지연을 고려해 t년 6월 시점 포트폴리오 구성(Fama-French 컨벤션)'이다. "
         + _BSK + _ASW +
         # 🚨 여기에 「…월말 리밸런스」 라는 글자를 쓰면 안 된다. 아래 _reb_text() 가
         #   규칙문·사유의 «월말 리밸런스» 를 그 규칙의 주기 이름으로 기계치환하는데,
         #   손으로 적은 새 주기 안의 «월말 리밸런스» 까지 다시 치환해
         #   «연 1회 6연 1회(6월말) 리밸런스» 가 산출물에 실렸다(2026-08-29 실측).
         #   치환기가 못 건드리는 표현으로 적는다.
         " 맞춘 것: 크기 52(십분위) · 리밸런스는 해마다 6월 마지막 거래일 한 번. "
         "🚨 이 규칙은 후보 풀이 얇은 달이 있다 — 10종판의 월별 후보가 중앙 458 이지만 "
         "최소 30 이다. 52를 못 채우는 리밸에는 '고른' 것이 아니라 '있는 것 전부'가 되고, "
         "그 해가 성적에 섞인다. 사전등록에 미리 적어 둔 사항이다. "
         "⚠ 연 1회라 10년 격자에서 리밸이 열 번뿐이다 — 회전율과 t 를 그 사실과 같이 읽을 것. "
         "⚠ 원문의 '극단적 자산급증·급감 특이치 필터'와 가치가중 판은 안 따랐다.",
         topn=52)
    ...
    xsec("x-realsw", "실질금리 국면 채점축 전환 (잉여현금흐름 ↔ 매출성장)",
         ...
         # 🚨 2026-08-29 — `topn=` 이 **없었다.** 이름·규칙문은 등록된 크기를 글자로 말하는데
         #   엔진은 TOPN(10)을 들고 있었다. 사전등록 PREREG-2026-08-23-NARRATIVE.md 가
         #   돌리기 전에 크기를 못박아 두었다 — A1 «fcfy 상위 20» · A2 «pb 하위 15» ·
         #   C1 «12개월 수익 상위 15 를 숏» · C2 «가장 음수인 20종». 그 문서의 정정 절이
         #   «안 바꾼 것 — 밴드(±0.5z)·**상위 20**·월말·대조군 전부 그대로» 라고 다시 적었다.
         #   즉 크기는 등록됐고 **배선만 빠졌다.** 이 저장소가 되풀이 밟는 «적어만 두고 안
         #   지킴» 이고, 규칙문과 계산이 엿새 동안 서로 다른 수를 말하고 있었다.
         # ⚠ 이것은 «결과를 보고 크기를 옮기는 것» 이 아니라 **등록된 수로 되돌리는 것**이다.
         #   그 구별이 이 랩의 전제라 여기 적어 둔다(사용자 지시 2026-08-29).
         topn=TOPN * 2)
    ...
    xsec("x-ratehot", "금리민감 과열 %d 숏" % (TOPN + 5),
         ...
         # ⚠ 이것은 «결과를 보고 크기를 옮기는 것» 이 아니라 **등록된 수로 되돌리는 것**이다.
         #   그 구별이 이 랩의 전제라 여기 적어 둔다(사용자 지시 2026-08-29).
         # ⚠ 금리민감 관문(|베타| 상위 3N = 30)은 이미 TOPN*3 으로 옳게 걸려 있었다.
         #   안 걸린 것은 **마지막 숏 바스켓 크기 15** 하나다.
         topn=TOPN + 5)
    ...
    xsec("x-revdrift", "투자의견 리비전 드리프트 (21일)",
         ...
         None, _REV_WHY)
    # ── 원 전략 크기 — PREREG-2026-08-30-BASKET2.md §2-2 ─────────────────
    xsec("x-revdrift-n25", "투자의견 리비전 드리프트 — 상위 25 (원 전략 크기)",
         "21일판과 같은 점수로 상위 25종목 동일가중, 월말 리밸런스.",
         None,
         "E21 리비전 드리프트의 원문은 '섹터 중립(GICS 섹터 내 순위) 후 상위 N=20~30 "
         "동일가중 롱'이다. 양 끝을 고르면 그것부터가 자유도이므로 중앙값 25를 쓴다 "
         "(A1 의 40-60 에서 50을 쓴 것과 같은 규약). "
         + "이 규칙은 짝이 되는 10종판과 점수 함수가 같다. 크기는 전략 탐색 풀의 원문에서 왔다 — "
         "이 랩이 고른 값이 아니다. 사전등록 PREREG-2026-08-30-BASKET2.md. "
         "⚠ 증분알파(incr5)의 최근접 이웃은 그 10종판으로 나올 것이 거의 확실하다 — "
         "같은 점수를 쓰므로 서로를 설명한다. 판정은 단독 t 와 부호로 읽을 것. "+
         "맞춘 것: 크기 25 · 월말 리밸 · 동일가중. "
         "🚨 원문이 정한 자동 실패 조건이 있다 — 카드가 '회전율 제약(월 회전 30% 캡). "
         "회전율이 통제되지 않으면 이 전략은 자동 실패로 판정해야 한다'고 적었다. "
         "짝인 10종판의 실측이 연 11.39회(리밸당 0.95)로 그 문턱의 세 배다. 이 판이 그 "
         "조건을 넘기는지가 성적보다 먼저 읽어야 할 수다. "
         "⚠ 섹터 중립은 안 따랐다 — 이 랩에 이미 다른 구성의 섹터중립판(x-revdrift-sn · "
         "섹터별 1위 1종)이 있어 같은 개념의 구현이 셋이 되고, 카드의 정제 필터(2노치 이상 "
         "변경 · 목표주가 동반 수정 · 브로커 규모 가중)를 랩 자료로 못 하는 채 포트만 원문에 "
         "맞추면 «원문대로» 라고 부를 수 없기 때문이다. "
         "⚠ 그 밖에 안 따른 것: 커버 애널리스트 3인 문턱 · 비대칭 홀딩(상향 1~3개월 / 하향 "
         "3~6개월) · 숏 다리.",
         topn=25)
    ...
    #   사용자가 2026-09-07 에 그 격자를 쓰지 말라고 정했다.
    #   ⚠ 등록 §3 이 **계산 전에** 「성적과 무관하게 내린다 · n52 가 더 나빠도 되살리지
    #     않는다」고 못박았다. 되살리면 성적으로 N 을 고른 것이 된다.
    #   ⚠ 정의는 아래 n52 판과 채점 갈래(x-residind 문자열)에 남는다.
    #     되살리려면 원장 항목에 readmitted 를 적어야 한다.
    xsec("x-residind-n52", "산업잔차 모멘텀 상위 52 (십분위 · 원 전략 크기)",
         "종목의 12-1 모멘텀에서 그 종목이 속한 GICS 산업그룹의 12-1(시총가중)을 뺀 잔차로 "
         "상위 52종목 동일가중, 월말 리밸런스. 직전 1개월은 건너뛴다. 산업그룹은 소속 종목이 "
         "8종 이상인 것만 쓴다 — 얇은 그룹이 기준이 되면 잔차가 «그 몇 종목 대비» 가 된다.",
         ...
         "🚨 2026-09-07 정정 — 그 분해를 선견 보정해 다시 쟀다"
         "(PREREG-2026-09-07-INDMOM2-RESULT). 「산업 성분이 매매 대상보다 못하다」는 "
         "여전히 맞지만(dS −0.106), 「그래서 빼면 좋아진다」의 크기는 선견이었다 — "
         "잔차가 산업을 앞선 폭이 소급 +0.301 에서 부분 PIT −0.001 로 사라졌다. "
         "같은 창에서 셋(종목·산업·잔차) 다 동일가중 유니버스에 진다. "
         "⚠ 이것은 그 분해의 잔차와도 다른 물건이다(분해는 바스켓 60~120종). "
         "그러나 이 규칙을 만든 근거가 그만큼 약하다. "
         "🚨 N 의 출처 — 신호는 Asness·Porter·Stevens(2000)의 within-industry momentum"
         "(개별 수익 − 자기 산업 평균)이지만 그 논문은 파마–맥베스 횡단면 회귀라 "
         "포트폴리오를 안 만든다. 즉 원문에 N 이 없다. 그래서 정렬 문헌의 표준인 십분위를 "
         "쓴다 — Jegadeesh·Titman(1993)이 십분위로 최상 데실을 롱하므로 52(=518÷10)다. "
         "랩의 x-mom12-n52·x-residmom-n52 와 같은 규약이고, 종전 10종판은 근거가 "
         "랩 기본값뿐이라 내렸다(PREREG-2026-09-07-RESIDIND2 §3). "
    ...
    # ── 2026-08-11 배치가 놓친 원 전략 크기 2종 — PREREG-2026-08-30-BASKET2.md ──
    # 🚨 그 배치는 카드의 lab 필드를 단서로 짝을 지었는데, E7·E34 는 그 칸이 비어 있고
    #   E21 은 lab 이 바스켓이 아니라 오버레이를 가리킨다 — **연결 고리가 없는 카드는
    #   훑기에서 통째로 안 보였다.** 성적을 보고 뺀 것이 아니라 도구의 사각지대였다.
    # ⚠ 짝이 되는 10종판(x-poacc · x-revdrift)은 **한 글자도 안 건드린다.** 2026-08-11
    #   규약 그대로다 — 결과를 보고 크기를 고치면 그 검정이 무효다.
    xsec("x-poacc-n52", "퍼센트 영업발생액 회피 — 최저 52 (십분위 · 원 전략)",
         "(순이익 − 영업현금흐름) ÷ |순이익| 이 가장 낮은 52종목 동일가중, 월말 리밸런스. "
         "둘 다 최근 12개월치를 쓴다.",
         None,
         "E7 발생액 아노말리의 원문은 '전 종목 데실(10분위) 정렬 → 최저 발생액 데실 롱 … "
         "동일가중이 표준 … 보유 1년'이고, 리밸은 '연 1회(회계연도 종료 후 재무제표 확정 "
         "시점, 통상 4~6월 lag)'다. 518종의 십분위는 52다. "
         + "이 규칙은 짝이 되는 10종판과 점수 함수가 같다. 크기는 전략 탐색 풀의 원문에서 왔다 — "
         "이 랩이 고른 값이 아니다. 사전등록 PREREG-2026-08-30-BASKET2.md. "
         "⚠ 증분알파(incr5)의 최근접 이웃은 그 10종판으로 나올 것이 거의 확실하다 — "
         "같은 점수를 쓰므로 서로를 설명한다. 판정은 단독 t 와 부호로 읽을 것. "+
         "맞춘 것: 크기 52(십분위) · 리밸런스는 해마다 6월 마지막 거래일 한 번 · 동일가중. "
         "🚨 N 과 주기를 같이 바꾼다 — 짝(10종·분기)과의 차이를 «크기 탓» 으로도 «주기 탓» "
         "으로도 읽을 수 없다. 등록에 미리 적어 둔 사항이다. "
         "⚠ 연 1회라 10년 격자에서 리밸이 열 번뿐이다. 회전율과 보유 종목 수는 믿을 만하고 "
         "샤프·t 는 구간에 크게 흔들린다 — 성적으로 읽지 말 것. "
         "⚠ 원문의 숏 다리(최고 발생액 데실)와 밸류 결합(per_f12m)은 안 따랐다.",
         topn=52)
    ...
    # ⚠ 여기는 밴드 변형(-band)보다 **앞**이어야 한다 — 변형이 dict(밑동) 복사로
    #   만들어지므로 뒤에 두면 변형이 밑동의 주기를 못 물려받는다.
    #   (종전에는 «NSWEEP 보다 앞» 이라고 적혀 있었다. 그 스윕은 2026-08-29 에 폐기됐고
    #    같은 성질을 가진 변형은 이제 밴드뿐이다.)
    ...
    REB_QE = {                              # 분기 — 입력이 분기에 한 번만 바뀐다
        ...
        "x-valcomp", "x-valcomp-sn", "x-btp-n155", "x-payout-n50",
    }
    # 🚨 연 1회(6월말) — PREREG-2026-08-29-ASWRITTEN.md §2. 하나뿐이다.
    #   E8 원문이 «연 1회, t년 6월 시점(Fama-French 컨벤션)» 이라고 못박아 두었는데
    #   2026-08-11 배치는 «안 따른 것» 표에 그것을 적어 두고 분기로 돌리고 있었다.
    #   ⚠ x-agrow(10종판)는 **안 옮긴다.** 그쪽은 이 랩의 규칙이고 원문 재현 대상이 아니다 —
    #     같이 옮기면 두 판의 차이가 크기 때문인지 주기 때문인지 안 갈린다.
    REB_Y6 = {"x-agrow-n52",
              # E7 발생액 — 카드가 «연 1회 … 통상 4~6월 lag … 보유 1년» 이라 적었다
              #   (PREREG-2026-08-30-BASKET2.md §2-1). 짝인 x-poacc 는 분기 그대로 둔다.
              "x-poacc-n52"}
    ...
    for _s in STRATS:
        if _s["kind"] != "xsec":
            continue
        _k = ("we" if _s["sid"] in REB_WE else
              "qe" if _s["sid"] in REB_QE else
              "y6" if _s["sid"] in REB_Y6 else None)
        if not _k:
            continue
        _s["reb"] = _k
    ...
    _reb_ghost = sorted((REB_WE | REB_QE | REB_Y6) - _reb_seen - DROPPED)
    if _reb_ghost:
        raise SystemExit("리밸 주기 표에 있으나 전략 목록에 없는 sid: %s — "
                         "PREREG-2026-08-13-REBAL.md 와 맞출 것" % ", ".join(_reb_ghost))
    print("  리밸 주기 — 주간 %d종 · 분기 %d종 · 연1회 %d종 · 월말 %d종 "
          "(PREREG-2026-08-13-REBAL · 연1회는 -ASWRITTEN)"
          % (len(REB_WE), len(REB_QE), len(REB_Y6),
    ...
    # ── 바스켓 크기 전수 시험을 **폐기했다** — PREREG-2026-08-29-ASWRITTEN.md §0 ────
    # 🚨 2026-08-29 사용자 지시(「전략이 말하는 그대로 하라」). 종전에는 횡단면 규칙 전부를
    #   N∈{10,20,30} 으로 돌리고 **샤프가 가장 높은 N** 을 골라 게시했다. 그 자리에 붙어
    #   있던 자백을 그대로 옮겨 둔다 —
    #     「샤프가 가장 높은 N 을 골랐다 … ⚠ 셋 중 최고를 고른 값이므로 이 규칙의 t 는
    #      그만큼 부풀려져 있다 — inflate 0.04」
    #   실측 예: x-sp 는 N=10 샤프 0.989 · N=20 0.939 · N=30 0.843 중 **10을 골라** 게시돼
    #   있었다. 성적을 보고 고른 자유도이고, 이 등록이 그것을 걷는다.
    # 🚨 **폐기이지 문턱 변경이 아니다.** 격자에서 하나를 고르는 절차 자체가 없어졌다.
    #   그래서 NSWEEP_GRID 상수 · `~n` 변형 생성 · 승자 선택 블록(`nsel`)이 전부 사라졌고,
    #   횡단면 규칙은 이제 **자기 topn 아니면 TOPN(10)** 하나만 든다.
    # ⚠ sid 집합은 안 바뀐다 — 승자는 어차피 밑동 sid 로 되돌려 실렸다. 바뀌는 것은 **수치**다
    #   (N=20·30 을 들고 있던 규칙이 10 으로 돌아온다). 게시 수치가 전부 다시 계산된다.
    # ⚠ NSW_SKIP 도 같이 없앴다. 「스윕에서 뺄 규칙」 목록은 스윕이 없으면 가리킬 것이 없다.
    #   그 목록이 담고 있던 사실(바스켓이 N 으로 안 정해지는 규칙이 있다)은 WEIGHTED_SIDS ·
    #   LONGSHORT_SIDS · CAP_ALL 이 이미 각자 들고 있다.
    ...
    #   은퇴 결정이 조용히 뒤집힌 것이고, 실측으로 잡았다. 그래서 `split("~n")[0]` 로 밑동을
    #   구해 검사하게 고쳤었다.
    # ⚠ 2026-08-29 — 스윕을 폐기해 `~n` 변형이 더는 안 만들어지므로 자름을 걷고 sid 를
    #   그대로 검사한다. **되살아나던 통로가 사라진 것이지 이 검사가 필요 없어진 것이 아니다** —
    #   새 접미사를 더하는 날 이 자리를 다시 볼 것(_BASE_SID 머리말과 같은 취지).
...
def xsec_score_at(S, i, X, pool=None):
    ...
    # ⚠ 종전에 여기서 바스켓 크기 변형(`~n20`·`~n30`)의 접미사를 벗겼다. 그 스윕은
    #   2026-08-29 에 폐기됐다(PREREG-2026-08-29-ASWRITTEN.md §0) — 벗길 접미사가 없다.
    #   🚨 그때 적어 둔 교훈은 살려 둔다: **접미사를 떼는 순서가 중요하다.** 순서가 틀리면
    #     'x-hlspread-mcf~n20' 이 endswith('-mcf') 에 안 걸려 시총 하한이 조용히 안 걸리고,
    #     갈래도 못 찾아 S["fn"](=None 인 2단 규칙)을 부르다 죽는다. 실제로 그렇게 죽었다.
    #     남은 접미사(-n52 · -band · -mcf)를 다룰 때 같은 함정을 다시 밟지 말 것.
    ...
    for t in tickers:
        ...
        if sid == "x-52wh":
            ...
            # ⚠ 여기 sid 는 _BASE_SID 를 거친 **밑동**이다(위 7196 줄). 등록 sid 는
            #   x-residind-n52 지만 «-n52» 는 크기 변형 표식이라 벗겨진 뒤 들어온다.
            #   🚨 2026-09-07 에 이것을 «x-residind-n52» 로 바꿨다가 갈래가 아예 안 타서
            #     PIT 이 전 구간 무보유가 됐다. 밑동으로 거는 것이 규약이다.
        ...
        # 유동성 수준 2종 — PREREG-2026-08-12-LIQ-CAL.md.
            ...
            if sid == "x-btp":
                ...
                # ⚠ 2026-09-03 — **이 갈래는 지금 안 돈다.** 규칙의 등록을 내렸다
                #   (사전등록 기각 A1FIX F2 · PIT t 1.24 — build_strats 의 그 자리 주석 참조).
                #   지우지 않는 이유는 이것이 규칙의 **정의**이고, 되살릴 일이 생기면
                #   사전등록 §1 을 코드로 옮긴 이 블록이 그 근거이기 때문이다.
                #   되살리려면 build_strats 의 xsec 호출 · SCREEN_SIDS · FUND_SIDS(tech·pit)
                #   넷을 같이 되돌리고, tested_not_published 의 항목에 readmitted 를 적어야 한다.
    ...
    # ⚠ **밑동으로** 건다(랩 규약 — 파일 머리말의 «sid 를 다루는 자리는 _BASE_SID 로»).
    #   지금 등록은 x-residind-n52 하나뿐이지만, 크기 변형이 늘어도 이 줄은 그대로 돈다.
    if _BASE_SID(S["sid"]) == "x-residind" and ind_raw:
...
def xsec_pick_at(S, i, X, sc, ind_raw, held=None):
    ...
    if S["sid"] == "x-indmom":
        ...
        return [t for t, _w in _pw], {t: w for t, w in _pw}
    # ── 원 전략을 「그대로」 — PREREG-2026-08-29-ASWRITTEN.md §2 ────────────────
    # 🚨 이 둘은 2026-08-11 배치가 «N 만 바꾸고 가중·중립화는 안 따랐다» 고 표에 적어 둔
    #   바로 그 열이다. 이번 등록이 그 열을 구현한다. **점수 함수는 한 글자도 안 바뀐다** —
    #   바뀌는 것은 고른 뒤의 비중과(E2), 고르는 방식과 비중이다(E3).
    if S["sid"] == "x-lowvol-n100":
        # E2 원문: "롱온리 변형은 저변동 상위 100종목 **역변동성 가중**. 월간 리밸런싱."
        # ⚠ 새 변동성을 다시 재지 않는다. 이 규칙의 점수가 이미 `-V`(60일 실현변동성의
        #   부호 반전)라, 비중은 그 점수에서 바로 나온다. 다시 재면 채점기와 비중이
        #   두 벌이 되고, 이 파일이 되풀이 밟은 사고가 정확히 그것이다.
        _hold = pick_top(sc, S["sid"], S.get("topn"))
        _sm = {t: v for v, t in sc}
        _w = {}
        for t in _hold:
            v = _sm.get(t)
            if v is not None and v < 0:            # v = −변동성 이므로 음수여야 한다
                _w[t] = 1.0 / (-v)
        # 🚨 전부 못 구하면 **동일가중으로 조용히 떨어지지 않는다.** 그러면 이름은
        #   «역변동성 가중» 인데 실제는 동일가중이 되고, 그 거짓이 화면까지 간다.
        #   비중을 못 구한 종목은 바스켓에서 뺀다(남은 것으로 되정규화된다).
        if len(_w) != len(_hold):
            _hold = [t for t in _hold if t in _w]
        return _hold, (_w or None)
    if S["sid"] == "x-btp-n155":
        # E3 원문: "상위 30% 롱 … 시가총액·섹터 중립화 권장. 월말~연 1회 리밸런싱,
        #   회전율 관리를 위해 히스테리시스 밴드 적용."
        # 🚨 「중립화」이지 「시총가중」이 아니다 — pick_neutral_2ax 머리말에 근거를 적었다
        #   (사용자 결정 2026-08-29). 두 축(섹터·시총 5분위)을 같은 잣대로 중립화한다.
        # ⚠ 밴드 폭은 이 랩에 이미 등록된 상수 하나(BAND=2.0)를 쓴다. 카드가 폭을 안 적었고,
        #   여러 값을 돌려 좋은 것을 고르면 그것이 곧 방금 없앤 자유도다
        #   (PREREG-2026-08-24-LOWTURN 이 «밴드 폭은 하나만 돌렸다» 로 정한 값이다).
        _sec = {t: (meta.get(t) or {}).get("sector") or "" for _v, t in sc}
        _mc = {}
        for _v, t in sc:
            _sn = asof_fund((FU.get(t) or {}).get("sh"), dates[i - 1])
            ...
            if _sn and _p0 and _sn > 0 and _p0 > 0:
                _mc[t] = _sn * _p0
        # ⚠ 비중은 안 준다(None = 동일가중). 두 축을 종목 비중으로 중립화하면 칸 비중이
        #   칸 자리 수로 나뉘어 1/n 로 상쇄되기 때문이다 — 머리말 참조.
        return pick_neutral_2ax(sc, _sec, _mc, (S.get("topn") or TOPN),
                                sid=S["sid"], held=held, band=BAND), None
...
def current_holdings(strats):
    ...
    ⚠ 이 함수는 **보유만** 만든다. 성과 수치는 한 글자도 안 건드린다 — 자른 격자에서 재고,
      ...
      성과' 라는 결정이 조용히 풀린다.
    ⚠ 횡단면 규칙의 선택(xsec_pick_at)은 **대체로** 상태가 없다 — 마지막 리밸 시점의 점수만
      있으면 된다. 반면 타이밍의 노출은 **상태를 이어 간다**(샹들리에 고점·KAMA·MACD 신호선)
      므로 전 구간을 한 번 돌려야 한다. 그래서 둘을 나눠 처리한다.
    🚨 2026-08-29 정정 — 「대체로」다. `needs_held()` 가 참인 규칙(밴드 변형 12종 +
      x-btp-n155)은 **이력현상이라 상태를 이어 간다.** 종전 문장이 «상태가 없다» 고 단언한
      탓에 이 함수만 held 를 안 넘기고 있었고, 그래서 밴드 규칙의 화면 명단이 곡선과
      갈려 있었다. 그 규칙들은 아래에서 리밸 이력을 처음부터 다시 걷는다.
    """
    ...
    try:
        for S in strats:
            ...
            try:
                ...
                if j is None:
                    continue
                # 🚨 2026-08-29 — **이력현상 규칙은 마지막 리밸 하나만 봐서는 못 만든다.**
                #   종전에는 여기서 `xsec_pick_at(..., ind_raw)` 를 held 없이 불렀다. 그러면
                #   pick_band 의 «들고 있던 것은 계속 든다» 가 빈 리스트를 받아 **그냥 상위 N**
                #   이 되고, 「밴드 적용」이라 적힌 규칙의 화면 명단이 곡선이 실제로 든 바스켓과
                #   갈린다. 실측(2026-08-29): x-btp-n155 는 155종 중 **30종**이 달랐고,
                #   게시 중인 x-lshock-band 도 10종 중 하나가 달랐다.
                #   ⚠ 이 랩이 되풀이 밟은 «경로가 둘이라 화면과 곡선이 갈림» 이고, 같은 사고를
                #     PIT 레그가 2026-08-25 에 이미 한 번 밟았다(그때는 PIT 만 고쳤다).
                #     run() · pit_backtest · 여기 — **세 경로 전부**가 held 를 넘겨야 끝난다.
                #   ⚠ 비용: 그 규칙만 리밸 이력을 처음부터 다시 걷는다. 대상이 13종뿐이라
                #     (밴드 12 + x-btp-n155) 전체 실행에 눈에 띄는 영향이 없다.
                _held = None
                if needs_held(S["sid"]):
                    _h = []
...
def run():
    ...
    for S in STRATS:
        ...
        if S["kind"] == "event":
            ...
            for i in range(MIN_HIST + 1, n):
                ...
                if (i - 1) in _rebset:
                    ...
                    #   🚨 2026-08-29 — `x-btp-n155` 가 여기 하나 더 붙는다. 접미사가 아니라
                    #     **규칙 안에** 밴드가 있는 첫 사례라 is_band() 로는 안 걸린다
                    #     (PREREG-2026-08-29-ASWRITTEN.md §2 — E3 원문이 밴드를 지정한다).
                    #     안 넘기면 「밴드 적용」이라 적힌 규칙이 매달 held=None 으로 들어가
        ...
        out.append({
            ...
            # 🚨 2026-08-29 — 여기 LONGSHORT_SIDS 가 빠져 있었다. 롱숏 둘은 선택 단위가
            #   **서브산업**이라 «상위 N 종목» 이라는 것이 없는데(그래서 별도 집합이다)
            #   tgt 10 을 찍고 있었다 — 실제 보유는 x-subls 평균 38.4종(33~45) ·
            #   x-subom 40.5종(36~45)이다. 화면이 바스켓 크기를 두 가지로 말하는 것이고,
            #   WEIGHTED_SIDS 가 애초에 막으려던 사고와 **같은 것**이다(그 머리말의
            #   x-indmom 실측 사례 참조). 두 집합을 다 봐야 한다.
            "bask": ((lambda bs, tgt: {"tgt": tgt, "n": len(bask_hist),
                         ...
                         (None if (_BASE_SID(S["sid"]) in WEIGHTED_SIDS
                                   or _BASE_SID(S["sid"]) in LONGSHORT_SIDS)
    ...
    # ── 바스켓 크기 승자 고르기를 **걷었다** — PREREG-2026-08-29-ASWRITTEN.md §0 ────
    # 🚨 2026-08-29. 여기 있던 것은 «같은 규칙의 N=10·20·30 세 판 중 샤프가 가장 높은 것을
    #   골라 밑동 sid 로 되돌려 싣는» 블록이었다. 고른 사실을 감추지 않으려고 셋을 다
    #   `nsel` 에 싣고 부풀림(max t − median t)까지 재서 같이 실었는데, 사용자 지시는
    #   **그 절차 자체를 없애라**는 것이다 — 재서 적어 두는 것과 안 하는 것은 다르다.
    # ⚠ 딸려 나간 것: `_NSW_MARK`(~n) · `nsel` 레코드 · 승자 개명(이름의 "상위 10" 을 고른
    #   N 으로 바꾸던 자리) · 부풀림 집계.
    # 🚨 처음에 여기에 «이제 모든 규칙이 등록 때 이름에 박은 수를 그대로 든다» 고 적었는데
    #   **거짓이었다.** 이름·규칙문에 TOPN 의 배수를 글자로 박아 놓고 `topn=` 은 안 넘기는
    #   규칙이 넷 있었다 — x-realsw(규칙문 "상위 20") · x-curvebank("…15") ·
    #   x-ratehot("…15") · x-fxweak("…20"). 넷 다 실제 바스켓은 TOPN(10)이었다.
    #   ⚠ **사용자 지시 2026-08-29 로 넷을 등록된 크기로 배선했다**(각 등록부의 주석 참조).
    #     이름을 10으로 내린 것이 아니라 바스켓을 20·15로 올렸다 — 그 크기는
    #     PREREG-2026-08-23-NARRATIVE.md 가 돌리기 전에 못박은 값이고, 빠진 것은
    #     구현뿐이었기 때문이다. «적어만 두고 안 지킴» 을 지킨 쪽으로 되돌린 것이다.
    #   🚨 이 어긋남을 막으라고 있는 아래 정렬 패스는 `_tg == TOPN` 이면 통째로 건너뛴다 —
    #     그래서 넷을 **구조적으로 못 봤다.** 지금은 tgt 이 20·15라 패스에 걸린다.
    #     ⚠ 그래도 «이름에 박힌 수 ≠ 실효 N» 을 일반으로 막는 검사는 여전히 없다.
    #       tgt 이 TOPN 인 채로 이름만 다른 수를 말하는 규칙이 새로 생기면 또 못 잡는다.
    # ⚠ 이 블록이 «PIT 병합보다 먼저» 와야 했던 이유도 같이 사라졌다. 그 사유는
    #   승자가 아직 `x-hlspread~n30` 이라 밑동 sid 인 PIT 표와 안 맞던 것인데,
    #   이제 sid 가 처음부터 밑동이다.
    ...
    # ⚠ 이 줄은 종전에 «바스켓 승자» 집계 print 와 짝지어 있었고, 그 탓에 첫 줄이 죽은
    #   `if _fam:` 블록 안쪽 들여쓰기로 남아 있었다. 승자 집계를 걷으면서 같이 폈다.
    if _NDX_FIRED:
    ...
    doc = {
        ...
        "limits": [
            ...
            # 🚨 2026-08-29 — 종전 문장은 "화면의 N종은 그중 **바스켓 크기 승자만 남긴 것**"
            #   이었다. 그 절차(전수 시험)를 이 등록이 폐기했는데 문장만 남아 있었고,
            #   그대로 report.html 의 «랩 전체의 한계» 로 나가고 있었다 — 없는 절차를
            #   사이트가 계속 말하는 것이다(이 파일이 되풀이 경고한 «글자와 계산이 어긋남»).
            ("다중검정 — 규칙 %d개를 돌렸다(이번 실행에 등록된 %d개 + 돌렸다가 목록에서 내린 "
             "%d개. 화면의 %d종이 등록된 전부이고, 다중검정에서 세야 하는 "
            ...
            # ⚠ "횡단면은 월말 리밸런스" 였다. 2026-08-13 에 주간·분기가, 2026-08-29 에
            #   연 1회가 생겨 오래전부터 거짓이었다. 주기는 규칙마다 다르고 카드에 적힌다.
            "신호는 당일 종가로 계산해 다음 거래일부터 적용한다(선견 없음). "
            "횡단면의 리밸 주기는 규칙마다 다르다(주간·월말·분기·연 1회) — 카드의 규칙문에 적힌다.",
            # ⚠ "종목 전략은 상위 10종목만 들고 간다" 였다. 2026-08-11 원 전략 크기 6종
            #   (50·52·100·155종)이 생긴 뒤로 거짓이다. 기본값이 10인 것은 여전히 맞다.
            "종목 전략은 기본이 상위 10종목이다(원 전략 크기를 그대로 쓰는 6종만 50~155종). "
            "50종목은 사실상 지수라 규칙이 고르는 게 없다시피 하고, 화면에서 '지금 무엇을 들고 "
            "있나'도 읽히지 않는다. 대신 10종목은 표본이 얇아 결과가 더 시끄럽다 — 개별 "
            "성과보다 판정(구별 가능한가)을 먼저 볼 것.",
            regime["note"],
