...
#         "거래량과 매일 쌓인 거래량은 다른 것이라는 가정. 둘 다 통과하면 그 자체가 정보다.")
    ...
    xsec("x-a1payout", "자사주매입+배당성장 — 자격필터 통과분 전부 (A1 원문)",
         ...
         "🚨 2026-09-02 감사 정정 — 조건 ①의 자사주액은 «12개월»이 아니라 직전 회계연도 값이고, "
         "ttm2 의 TTM_STALE_DAYS(550일) 때문에 최대 455일까지 낡은 채 쓰인다(전 694 보유칸 중앙 272일 · "
         "1년 넘게 낡은 칸 15%). 선견은 아니지만(과거 쪽으로 낡았다) 사전등록문의 「12개월」과 다르다. "
         "2021년 넉 분기가 1~4종으로 마른 것도 그 시차가 2020년의 자사주 중단을 실제보다 오래 보여 준 탓이 크다. "
         "결과를 보고 창을 바꾸면 그것이 튜닝이라 여기서 고치지 않는다 — 새 사전등록의 일이다. "
         "🚨 성적을 액면대로 읽지 말 것: 40분기 중 11분기(28%)가 15종 미만이고, 그 얇은 구간을 빼면 "
         "Δ샤프 0.377 → 0.175 다. 특히 1종(KR)만 든 2022-01~03 분기는 초과 +31.9%p 중 88%가 "
         "2022-03-04 한 주에서 나왔다(그 주에 크로거 실적 발표). 자세한 것은 "
         "PREREG-2026-09-02-A1PAYOUT-RESULT.md §8.",
         topn=999, reb="qe")
